...
Object.assign(App, {
    ...
    // Uses Anthropic (preferred for deep reasoning) or Gemini to analyze coverage gaps
    async runCoverageGapAnalysis() {
        ...
        lines.push(`State: ${d.addrState || d.state || 'unknown'}`);
        ...
        if (hasProperty) {
            ...
            if (d.dwellingType) lines.push(`Dwelling: ${d.dwellingType}`);
            ...
            if (d.roofType) lines.push(`Roof: ${d.roofType}${d.roofYr ? ` (${d.roofYr})` : ''}`);
            if (d.heatingType) lines.push(`Heating: ${d.heatingType}`);
            ...
            if (d.dogBreed) lines.push(`Dog Breed: ${d.dogBreed}`);
            ...
            // Coverage limits
            if (d.dwelling) lines.push(`Dwelling Coverage: $${d.dwelling}`);
            if (d.liability) lines.push(`Liability: $${d.liability}`);
            if (d.deductibleAOP) lines.push(`AOP Deductible: $${d.deductibleAOP}`);
            if (d.medPay) lines.push(`Med Pay: $${d.medPay}`);
        }
        ...
        if (hasAuto) {
            ...
            if (d.bodInjury) lines.push(`BI Limits: ${d.bodInjury}`);
            if (d.propDamage) lines.push(`PD Limit: $${d.propDamage}`);
            if (d.umUim) lines.push(`UM/UIM: ${d.umUim}`);
            if (d.compDed) lines.push(`Comp Deductible: $${d.compDed}`);
            if (d.collDed) lines.push(`Collision Deductible: $${d.collDed}`);
        }

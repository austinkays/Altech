...
Object.assign(App, {
    ...
    async buildPDF(data) {
        ...
        const sectionHeader = (title) => {
            checkPage(16);
            // 7.5pt uppercase #555 label, letter-spaced, with 1px #ccc border-bottom flush below
            doc.setFontSize(7.5);
            ...
            y += 15; // 7px label + 1px rule + 8px gap below
        };

        // Key-value grid — stacked label/value cells, no internal borders
        // Label: 7pt uppercase #888 | Value: 10pt normal #111
        const kvTable = (fields, cols = 3) => {
            const filtered = fields.filter(([, v]) => v && String(v).trim());
            if (!filtered.length) return;
            const colW = contentW / cols;
            // Cell height: 3.5pt label + 1pt gap + 10pt value + 10pt padding-bottom = ~14.5pt
            const cellH = 14;
            let col = 0;
            ...
            filtered.forEach(([label, value]) => {
                ...
                // Label — 7pt, uppercase, #888, letter-spaced
                doc.setFontSize(7);
                doc.setFont(undefined, 'normal');
                doc.setTextColor(136, 136, 136); // #888
                doc.text(label.toUpperCase(), cellX, cellY + 3.5);

                // Value — 10pt, normal weight, #111
                doc.setFontSize(10);
                doc.setFont(undefined, 'normal');
                doc.setTextColor(17, 17, 17); // #111
                const maxValW = colW - 4;
                const valLines = doc.splitTextToSize(String(value), maxValW);
                doc.text(valLines[0] || '', cellX, cellY + 8);
                if (valLines[1]) doc.text(valLines[1], cellX, cellY + 12.5);

                col++;
                if (col >= cols) {
                    col = 0;
                    y += cellH + (valLines[1] ? 4.5 : 0);
                }
            });
            if (col > 0) y += cellH;

            y += 4; // 8px section gap below
        };
        ...
        // Document ref (8pt, monospace-style via Courier) & timestamp (7pt, #888)
        ...
        // Client info card + satellite side by side
        // Satellite: ~120x90px @ 96dpi = ~32x24mm in jsPDF points
        const satW = 32, satH = 25;
        const hasSat = !!(mapImages?.satellite?.dataUrl);
        const cardPad = 3;
        const infoW = hasSat ? contentW - satW - cardPad * 2 - 2 : contentW;
        const infoBoxH = Math.max(30, hasSat ? satH + 2 : 0);
        ...
        // Applicant name — 18pt bold
        doc.setFontSize(18);
        doc.setFont(undefined, 'bold');
        doc.setTextColor(...C.dark);
        doc.text(clientName, margin + cardPad, y + 10);

        // Address + coverage type — 10pt gray
        const policyTypeBadge = v('homePolicyType') ? `  \u2022  ${v('homePolicyType')}` : '';
        doc.setFontSize(10);
        doc.setFont(undefined, 'normal');
        doc.setTextColor(85, 85, 85); // #555
        const addrLine = doc.splitTextToSize((address || '') + policyTypeBadge, infoW - cardPad * 2 - 4);
        doc.text(addrLine[0] || '', margin + cardPad, y + 18);

        // Phone — 8.5pt #888
        doc.setFontSize(8.5);
        doc.setTextColor(136, 136, 136); // #888
        doc.text(formatPhone(v('phone')), margin + cardPad, y + 24);
        doc.setTextColor(...C.dark);

        // Satellite thumbnail (right inside card)
        if (hasSat) {
            const satX = margin + infoW + cardPad + 2;
            const satY = y + (infoBoxH - satH) / 2;
            ...
            doc.setFontSize(5.5);
            doc.setTextColor(170, 170, 170); // #aaa
            doc.text('Satellite View', satX + satW / 2, satY + satH + 3.5, { align: 'center' });
            doc.setTextColor(...C.dark);
        }

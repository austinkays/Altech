...
window.HawkSoftExport = (() => {
    ...
    function _buildFscNotes(d, drivers = []) {
        ...
        // Property Details
        const prop = [];
        if (d.dwellingType) prop.push(`${FIELD_BY_ID['dwellingType'].label}: ${d.dwellingType}`);
        if (d.dwellingUsage) prop.push(`${FIELD_BY_ID['dwellingUsage'].label}: ${d.dwellingUsage}`);
        if (d.occupancyType) prop.push(`${FIELD_BY_ID['occupancyType'].label}: ${d.occupancyType}`);
        ...
        if (d.roofType) prop.push(`${FIELD_BY_ID['roofType'].label}: ${d.roofType}`);
        if (d.roofShape) prop.push(`${FIELD_BY_ID['roofShape'].label}: ${d.roofShape}`);
        if (d.roofYr) prop.push(`${FIELD_BY_ID['roofYr'].label}: ${d.roofYr}`);
        if (d.heatingType) prop.push(`${FIELD_BY_ID['heatingType'].label}: ${d.heatingType}`);
        if (d.cooling) prop.push(`${FIELD_BY_ID['cooling'].label}: ${d.cooling}`);
        if (d.heatYr) prop.push(`${FIELD_BY_ID['heatYr'].label}: ${d.heatYr}`);
        if (d.plumbYr) prop.push(`${FIELD_BY_ID['plumbYr'].label}: ${d.plumbYr}`);
        if (d.elecYr) prop.push(`${FIELD_BY_ID['elecYr'].label}: ${d.elecYr}`);
        if (d.garageType) prop.push(`${FIELD_BY_ID['garageType'].label}: ${d.garageType}${d.garageSpaces ? ' (' + d.garageSpaces + ' spaces)' : ''}`);
        if (d.lotSize) prop.push(`${FIELD_BY_ID['lotSize'].label}: ${d.lotSize}`);
        if (d.numFireplaces && d.numFireplaces !== '0') prop.push(`${FIELD_BY_ID['numFireplaces'].label}: ${d.numFireplaces}`);
        if (d.pool) prop.push(`${FIELD_BY_ID['pool'].label}: ${d.pool}`);
        if (d.dogInfo) prop.push(`${FIELD_BY_ID['dogInfo'].label}: ${d.dogInfo}`);
        if (d.trampoline && d.trampoline !== 'No') prop.push(`${FIELD_BY_ID['trampoline'].label}: ${d.trampoline}`);
        if (d.woodStove && d.woodStove !== 'No') prop.push(`${FIELD_BY_ID['woodStove'].label}: ${d.woodStove}`);
        if (d.businessOnProperty && d.businessOnProperty !== 'No') prop.push(`${FIELD_BY_ID['businessOnProperty'].label}: ${d.businessOnProperty}`);
        if (d.smokeDetector) prop.push(`${FIELD_BY_ID['smokeDetector'].label}: ${d.smokeDetector}`);
        ...
        if (d.equipmentBreakdown && d.equipmentBreakdown !== 'No') endorse.push(`${FIELD_BY_ID['equipmentBreakdown'].label}: Yes`);
        if (d.serviceLine && d.serviceLine !== 'No') endorse.push(`${FIELD_BY_ID['serviceLine'].label}: Yes`);
        if (d.creditCardCoverage) endorse.push(`${FIELD_BY_ID['creditCardCoverage'].label}: ${d.creditCardCoverage}`);
        if (d.moldDamage) endorse.push(`${FIELD_BY_ID['moldDamage'].label}: ${d.moldDamage}`);
        if (d.theftDeductible) endorse.push(`${FIELD_BY_ID['theftDeductible'].label}: ${d.theftDeductible}`);
        if (d.additionalInsureds) endorse.push(`${FIELD_BY_ID['additionalInsureds'].label}: ${d.additionalInsureds}`);
        if (endorse.length) sections.push('ENDORSEMENTS\n' + endorse.join('\n'));

        // Insurance History
        const hist = [];
        if (d.priorCarrier) hist.push(`${FIELD_BY_ID['priorCarrier'].label}: ${d.priorCarrier}`);
        ...
        // Risk / Other
        const risk = [];
        if (d.accidents) risk.push(`${FIELD_BY_ID['accidents'].label} (global): ${d.accidents}`);
        if (d.violations) risk.push(`${FIELD_BY_ID['violations'].label} (global): ${d.violations}`);
        if (d.studentGPA) risk.push(`${FIELD_BY_ID['studentGPA'].label} (global): ${d.studentGPA}`);
        if (d.residenceIs) risk.push(`${FIELD_BY_ID['residenceIs'].label}: ${d.residenceIs}`);
        if (d.contactTime) risk.push(`${FIELD_BY_ID['contactTime'].label}: ${d.contactTime}`);
    ...
    // Row renderers live in hawksoft-renderers.js (window.HawkSoftRenderers).
            ...
            <div class="hs-section-body">
                ...
                <div class="hs-subsection">
                    ...
                    <div id="hs_vehicleList">
                        ${_exportData.vehicles.map((v, i) => HawkSoftRenderers.vehicleRow(v, i)).join('')}
                    </div>
                </div>
                <div class="hs-subsection">
                    <div class="hs-subsection-header">
                    ...
                    <div id="hs_driverList">
                        ${_exportData.drivers.map((d, i) => HawkSoftRenderers.driverRow(d, i)).join('')}
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="hs-section-body">
                ...
                <div class="hs-subsection">
                    ...
                    <div id="hs_coverageList">
                        ${cm.coverages.map((cov, i) => HawkSoftRenderers.coverageRow(cov, i)).join('')}
                    </div>
                </div>
            </div>
        </div>

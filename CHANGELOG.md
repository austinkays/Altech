...
### Added
- **fix(intake-v2): EZLynx EZAUTO "Error in deserializing to V200 Ezauto" — conform to the known-good sample** (May 19, 2026):
  A second failing export (same client) surfaced the *real* root cause behind the EZLynx rejection: a hard **V200 deserialization** error, i.e. a schema/structure violation — not the missing-DL data issue. Diffed the V2 EZAUTO output against the only file EZLynx provably accepts ([Resources/archive/HawkSoft Export to EZLynx SAMPLE.xml](Resources/archive/HawkSoft%20Export%20to%20EZLynx%20SAMPLE.xml)) and found three structural deviations the .NET deserializer rejects, all now fixed in [js/intake-v2-export-ezlynx.js](js/intake-v2-export-ezlynx.js): **(1) `<PrincipalVehicle>` inside `<Driver>`** — the known-good sample has **zero** of these; it was an unknown child element. Removed (EZLynx infers the driver↔vehicle link from `<VehiclesUse>`/`<VehicleAssignments>`). **(2) `<DriverAssignment id="N"/>`** — the sample emits an **empty `<DriverAssignment/>` with no `id` attribute**; the extra attribute was rejected. Now matches. **(3) `<Rated>Not Rated</Rated>`** — the sample's `<Rated>` is only ever `Rated`; "Not Rated" is not in the V200 enum (this was a regression from the prior commit's well-intentioned but invalid degrade). Reverted to always `Rated` — a driver with no DL# still deserializes fine (it just won't *rate* in EZLynx), and the pre-export validator already warns the agent to add the DL#. Also dropped `<PrincipalOperator>` from `<VehicleUse>` (another dangling cross-ref absent from the sample) and removed now-dead `driverIdxById`. Net: V2 EZAUTO is now structurally identical to the proven-good sample. Validator/orchestrator warning copy corrected ("imports but won't rate" rather than the now-inaccurate "Not Rated"). The `IntakeV2 — EZLynx import safety` suite ([tests/intake-v2.test.js](tests/intake-v2.test.js)) now asserts the sample-shape invariants (no `<PrincipalVehicle>`/`<PrincipalOperator>`, no `id` on `<DriverAssignment>`, no "Not Rated"). Full suite **66/67, 2502 passing** (lone failure the pre-existing unrelated `intake-v2` Progressive-bindability test); `lint-aad` + `audit-docs` clean. **Known follow-up:** the legacy V1 EZAUTO builder ([js/app-export-acord-xml.js](js/app-export-acord-xml.js)) has the *identical* `<PrincipalVehicle>` + `<DriverAssignment id>` deviations and would fail the same way — left untouched here pending a decision (battle-tested path, broad test churn) and flagged to the user.

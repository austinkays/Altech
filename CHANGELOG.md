...
### Changed
- **feat(intake-v2): EZLynx-format dropdown parity + industry/occupation cascade + utility-field Smart Scan capture, and tile demoted to "In Development"** (May 19, 2026):
  Major v2 catch-up after a user pointed out three regressions vs. v1 on the same address: dropdown values that didn't match the EZLynx Personal Lines application enums (so the exporter would silently lose fields if anyone hooked it up), Industry/Occupation degraded from select cascades to plain text inputs, and Smart Scan dropping Sewer / Water / Fireplaces / Flooring that v1 captured from the Rentcast/Redfin/Zillow scrape. All four addressed without touching v1.
  - **Tile position**: `intakev2` moved from the "Intake & Quoting" cluster to the "In Development (Beta)" section in [js/app-init.js](js/app-init.js) `toolConfig`. v1's Personal Intake is now the obvious default, v2 stays available for ongoing development.
  - **EZLynx-aligned options** ([js/intake-v2-fields.js](js/intake-v2-fields.js)): new `PREFIX_OPTIONS_EZ` / `SUFFIX_OPTIONS_EZ` / `GENDER_OPTIONS_EZ` / `MARITAL_OPTIONS_EZ` / `EDUCATION_OPTIONS_EZ` / `INDUSTRY_OPTIONS_EZ` constants whose values mirror the EZLynx app strings exactly. `prefix` / `suffix` / `gender` use `[code, label]` tuples (`MR` / `Mrs.`) so the agent sees the friendly label while storage carries the EZLynx code. Applied to **applicant, co-applicant, AND operator** fields. Maritime status gained `Separated` (an EZLynx enum v2 was missing). Education swapped 7 short labels for the full 10-option EZLynx list (`"No High School Diploma"`, `"Phd"`, `"Law Degree"`, etc.).
  - **Industry → Occupation cascade**: `OCCUPATIONS_BY_INDUSTRY_EZ` (24-key, ~250-occupation map mirrored from v1's `App._OCCUPATIONS_BY_INDUSTRY`) exported via `window.IntakeV2Fields.OCCUPATIONS_BY_INDUSTRY`. `applicant.industry` + `applicant.occupation` both convert from `type: 'text'` to `type: 'select'`; the Occupation field carries a new schema property `cascadeFrom: 'applicant.industry'` and ships with `options: ['']` because the real list is populated at runtime. Co-applicant gets the matching pair. Operator-level "Occupation" stays a single dropdown of industry CATEGORIES (matches v1's Step-4 driver row + how the EZLynx exporter expects per-driver records).
  - **Cascade wiring** ([js/intake-v2-layout.js](js/intake-v2-layout.js)): new `wireIndustryOccupationCascade()` delegates a `change` listener on the v2 container so industry-select changes repopulate every dependent `cascadeFrom` field. `repaintOccupationOptions()` runs at boot (after `applyData` restores values) and after every `requestRerender('layout')` so the Occupation dropdown always reflects the current industry. If a previously-saved occupation isn't in the new industry's list it's cleared from data (so what's in storage matches what's on screen).
  - **Smart Scan field expansion** ([js/intake-v2-fields.js](js/intake-v2-fields.js) home schema + [js/intake-v2-core.js](js/intake-v2-core.js) home `itemDefaults` + [js/intake-v2-smart-fill.js](js/intake-v2-smart-fill.js) `_readZillow` + `NORMALIZE`): four new fields on every home — `systems.sewer` (Public/Septic/None), `systems.water` (Public/Well/Cistern/None), `systems.flooring` (text), `systems.fireplaces` (number). `_readZillow` now picks `sewer/sewerType/sewer_system`, `water/waterSource/water_source`, `flooring/floor_type/flooringType`, `fireplaces/fireplace_count/numFireplaces` from the Rentcast + Apify scrape response. `NORMALIZE['systems.sewer']` and `['systems.water']` map upstream phrasings ("Public Sewer", "Private Well") to the dropdown values. The same Rentcast call that returned all 16 fields for v1 now surfaces them in v2's Smart Scan modal.
  - Test coverage: 1 new regression test in [tests/intake-v2-smart-fill.test.js](tests/intake-v2-smart-fill.test.js) asserting Sewer/Water/Flooring/Fireplaces survive `mergeResults`. Smart-fill suite: 28 tests (was 27).
  - **Compatibility note**: per the user's explicit choice, no migration runs against existing v2 draft data with the old short labels. Any draft with `education: 'No HS'` or `gender: 'Male'` will show a blank dropdown until re-picked; new sessions get the EZLynx-aligned values from the start.

### Fixed
- **fix(intake-v2): Smart Scan returned 2 fields when v1 returned 16 for the same address — `parseAddress` mis-read legacy county-at-end layout** (May 19, 2026):
  Real user report. Comparing v1 vs v2 on `180 Drews Prairie Rd, Toledo, WA 98591` showed v1 pulling 16 fields from Rentcast + Apify Redfin + Apify Zillow + Fire, while v2 returned only 2 fields from Fire / Protection Class. Root cause was in [js/intake-v2-smart-fill.js](js/intake-v2-smart-fill.js) `parseAddress`. The user's home was saved with the LEGACY `_formatAddressLine` order (`"Street, City, State Zip, County"`) — county AT END. The position-based parser walked backward expecting state-zip last and mis-read `"Lewis County"` as the state-zip slot → state captured as `"LE"` (first two letters of "Lewis"), ZIP came out empty, the county slot got the actual `"WA 98591"`. v2 then sent `{state: "LE", zip: ""}` to `/api/property-intelligence?mode=zillow` and Rentcast / Apify Redfin / Apify Zillow all returned nothing — only Fire/PC tolerated the garbage. (Previous PR #179 reordered `_formatAddressLine` so NEW Places picks come out correctly, but homes saved before that fix still carry the legacy layout in storage — and the parser has to handle them.) Replaced the position-based parser with a pattern-based one: scan right-to-left for any segment matching `/^[A-Z]{2}\s*(\d{5}(-\d{4})?)?\s*$/` and lock that as state-zip regardless of position; country tags (`USA` / `United States`) are filtered up-front; anything left over is treated as a county candidate with `" County"`-suffix matches preferred. All six observed layouts now parse identically — `Street, City, State Zip`, `Street, City, County, State Zip`, `Street, City, State Zip, County`, each with and without trailing `, USA`. Added 3 regression tests in [tests/intake-v2-smart-fill.test.js](tests/intake-v2-smart-fill.test.js); the smart-fill suite is now 27 tests (was 24). No schema change — existing saved home addresses don't need migration, the parser handles them in place.
- **fix(intake-v2): Smart Scan got `state: 'KI', zip: ''` from Places-picked addresses — county was in the wrong position** (May 19, 2026):
  When the agent picked a property address from Google Places autocomplete, [js/intake-v2-places.js](js/intake-v2-places.js) `_formatAddressLine` wrote it as `"Street, City, State Zip, County"` (county at the END). But `parseAddress` in [js/intake-v2-smart-fill.js](js/intake-v2-smart-fill.js) walks BACKWARD to find state-zip (the last segment, modulo a `, USA` country tag), so it treated `"King County"` as the state-zip → state captured the first two letters as `"KI"`, ZIP came out empty, the county slot got the actual `"WA 98101"`. Smart Scan then sent `{ state: 'KI', zip: '' }` to `/api/property-intelligence` and every source (ArcGIS, Rentcast, Fire Station, Vision) returned nothing useful — surfaced as the "Smart Scan didn't find much" report. Fixed by reordering `_formatAddressLine` to put county BEFORE state-zip (`"Street, City, County, State Zip"`), matching the format `parseAddress` documents in its own comment AND the existing `'accepts a county hint between city and state'` test. Added a regression test in [tests/intake-v2-smart-fill.test.js](tests/intake-v2-smart-fill.test.js) that round-trips a Places-shaped object through `_formatAddressLine` → `parseAddress` and asserts state='WA' / zip='98101' / county='King County' (the bug shape would've produced state='KI' / zip=''). 24 tests in the smart-fill suite (was 23).

### Added
- **feat(intake-v2): auto-fill Property Address from Mailing Address — button + first-home pre-fill** (May 19, 2026):
  The personal intake v2 had two address blocks that overlap in the common case ("insure the home I live in") but had to be typed twice: the structured Mailing Address (`address.street/city/state/zip`) and the freeform Property Address on the home card (`home.address`, a single comma-formatted string). Two ergonomic wins: **(1)** Clicking **+ Home** now pre-fills the first home's Property Address from the mailing fields when those are filled out enough to parse (`street + city + state` minimum) — formatted `"street, city, state zip"` to match `intake-v2-smart-fill.js`'s `parseAddress` expectation (3+ comma-separated parts). Second/third homes (vacation, rental) stay blank because they live somewhere else by definition. **(2)** A subtle **"Same as mailing"** link appears next to the Property Address label on any home card where the mailing address is available AND doesn't already match the home — clicking it copies the formatted string into the input and dispatches an `input` event so the standard save listener picks it up exactly as if the agent typed it. The hide-when-equal compare is case-insensitive so a casing-only difference doesn't keep a no-op button visible. Side benefit: this also fixes the most common cause of "Smart Scan didn't find much" — Smart Scan refuses to run when `parseAddress` returns null (fewer than 3 comma-separated parts), and a typed address without commas was the typical reason; auto-fill produces a clean comma-formatted string that parses every time. Files: [js/intake-v2-property.js](js/intake-v2-property.js) (private `_mailingAddressString`, modified `addressHeader`, new click wiring in `renderHomes`), [js/intake-v2-layout.js](js/intake-v2-layout.js) (mirror `_mailingAddrString` + first-home pre-fill in the `[data-add]` handler — duplicated rather than coupled because layout.js loads before property.js), [css/intake-v2.css](css/intake-v2.css) (new `.iv2-field-label-row` flex container + `.iv2-same-as-mailing-btn` text-link styling).
- **feat(security): governance close-out — one-page Principal Acknowledgment + incident-response line in the PDF** (May 19, 2026):
  Audit feedback on the Security page (which both reviewers called "technically excellent") flagged two governance items genuinely worth doing at one-person-internal-tool scale, and a long list that's calibrated for a SaaS vendor selling to multiple agencies and is **deliberately skipped** here. **(1)** New [docs/internal-tools-acknowledgment.html](docs/internal-tools-acknowledgment.html) — a print-friendly one-pager naming the tool, purpose, developer of record (Austin Kays), in-use-since date (March 2026), with a signature line for the principal (Neil Worland, Owner, Altech Insurance Agency). Print → sign once → file with internal records. Closes the "is this tool authorized for use at the agency?" audit question. **(2)** Added one accurate bullet under *Data handling & your control* in the in-app Security page: *"Incident response: in the event of a suspected breach the principal is notified and the Washington RCW 19.255 notification timeline is followed."* Closes the "what happens if something goes wrong?" question with a defensive statement (not an external filing — no E&O involvement, no DPA inventory, no sub-processor table; explicitly out of scope at this scale). Extended [tests/security-info-page.test.js](tests/security-info-page.test.js) (+2 tests: the incident-response bullet is present; the acknowledgment doc exists, names the right principal + developer + date, and is print-friendly with a signature line). Full suite 73/73, **2652 tests**; `audit-docs` clean.
- **fix(security): Security PDF spilled 2 bullets + footer onto a 3rd page — tighten print spacing to fit 2 pages** (May 19, 2026):
  After the portrait switch, the auditor PDF was *just barely* 3 pages — page 3 held only the last 2 "Data handling" bullets + the footer (the rest was empty whitespace). Tightened the print-only spacing to absorb the overflow: `@page` margin 0.5in → 0.4in in [js/app-boot.js](js/app-boot.js)'s injected style; and in [css/security-info.css](css/security-info.css)'s `@media print` block, denser plain-language cards (`.secinfo-section` 14px → 8px padding, icons 36→28px), tighter technical-detail bullets (`.secinfo-list li` margin-bottom 5→3px, line-height 1.65→1.45), tighter `<h5>` margins (18→10px top), and slightly tighter header/body padding. Screen rendering unchanged — all overrides are inside `@media print` and gated on `body.secinfo-printing`. Full suite 73/73, 2650 tests; `audit-docs` clean.
- **feat(security): Security PDF prints portrait instead of landscape** (May 19, 2026):
  Print / Save PDF was rendering landscape (and would otherwise need 3 pages of content). Cause: [css/task-sheet.css](css/task-sheet.css) ships an unscoped `@page { size: letter landscape }` inside its `@media print` block, and `@page` can't be CSS-selector-scoped, so that landscape directive bleeds into every print job in the app. Fix: `SecurityInfo.print()` ([js/app-boot.js](js/app-boot.js)) now appends a `<style id="secinfo-print-page">@media print { @page { size: letter portrait; margin: 0.5in; } }</style>` immediately before `window.print()`. Appended last in source order, so its `@page` wins for this print only; the cleanup on `afterprint` removes the style so the Task Sheet print keeps its own landscape setup. Extended [tests/security-info-page.test.js](tests/security-info-page.test.js): source-string asserts the injection + removal contract, and the behavioral JSDOM test now also verifies the `#secinfo-print-page` style is live in `<head>` during print and gone after `afterprint`. Suite 73 (extended); `audit-docs` clean.
- **fix(security): Print / Save PDF was still blank — overlay isn't a direct body child at runtime; hoist it for the print** (May 19, 2026):
  After the `focus`-race fix the export was *still* blank. Live-DOM diagnostic (`document.getElementById('securityInfoOverlay').parentElement.tagName`) returned `"DIV"`, not `"BODY"` — the static `#securityInfoOverlay` ends up nested inside an app-shell wrapper at runtime (boot/init re-parents overlays). The `@media print` isolation in [css/security-info.css](css/security-info.css) uses `body.secinfo-printing > *:not(#securityInfoOverlay){display:none}` to collapse every other body child so only the overlay's content prints — but when the overlay is nested, that selector hides its **wrapper**, which hides the overlay with it, and the PDF rasterizes blank. (This was the root cause behind the entire blank-page chain, not the `focus` race — the `focus` race was a real but lesser bug that masked it.) Fix: `SecurityInfo.print()` ([js/app-boot.js](js/app-boot.js)) now **hoists the overlay to be a direct child of `<body>` for the duration of the print and restores it to its original parent + next-sibling on `afterprint`** — so the structural CSS selector matches the live DOM regardless of where boot code parked the overlay. Pure DOM move; preserves listeners, doesn't touch CSS. Extended [tests/security-info-page.test.js](tests/security-info-page.test.js) with a **behavioral JSDOM test** that builds a nested-wrapper DOM, calls `SecurityInfo.print()`, asserts the overlay is hoisted to body during print and restored to the wrapper after `afterprint` — the strongest possible regression guard for this exact bug. Full suite 73/73, **2649 tests**; `audit-docs` clean.
- **fix(security): Print / Save PDF intermittently blank — drop the `focus` cleanup race** (May 19, 2026):
  After the trailing-blank-pages fix the export was *intermittently* fully blank (1 page, no content) even when triggered from the in-page button — one run at `#compliance` rendered, the next at `#home` was blank. Root cause: the `focus` cleanup backstop added in #171. `SecurityInfo.print()` ([js/app-boot.js](js/app-boot.js)) registered `window.addEventListener('focus', () => setTimeout(cleanup, 0))` — but focus returns to the window the moment the print **preview** opens, which for "Save as PDF" is well *before* the file is actually rasterized (saving is deferred until the user picks a location). So `cleanup` stripped `body.secinfo-printing` mid-preview and the PDF saved blank — the exact same failure mode as the original 2 s timer, just triggered by `focus` and timing-dependent (hence intermittent). Fix: cleanup is now **`afterprint`-ONLY** — the only event that fires *after* rasterization. No timer, no focus, no early-fire backstop. If a browser never fires `afterprint` the class merely lingers (it only has effect inside `@media print`; the overlay closes via Esc/✕) — a benign failure vs. a blank export. Updated [tests/security-info-page.test.js](tests/security-info-page.test.js) to assert cleanup is afterprint-only and that neither a `setTimeout` nor a `focus` listener is reintroduced (the two backstops that each blanked the output). JS+test only; full suite 73/73, 2647 tests; `audit-docs` clean.
- **fix(security): Print / Save PDF emitted ~6 trailing blank pages** (May 19, 2026):
  Follow-up to the print-blank-page fix. The content now renders correctly (verified 3-page auditor doc) but the PDF was **9 pages — pages 4-9 blank**. Regression from that fix's isolation choice: hiding the rest of the app with `visibility:hidden` ([css/security-info.css](css/security-info.css) `@media print`). `visibility:hidden` keeps an element's **layout box**, so the whole app's height (dashboard, sidebar, ~20 tool containers) still generated ~6 empty pages after the content. Switched the sibling-hide to **`display:none`** (collapses the boxes → zero height → no extra pages) and made the overlay **`position:static`** so its content flows and paginates normally (the previous `position:absolute` also mis-paginates/clips multi-page content in print). `visibility` is kept only as a cheap backstop. `display:none` on siblings is safe now because #171 already deconflicted the unscoped `task-sheet.css` print rule (it excludes `#securityInfoOverlay`) and removed the premature print-class timer, and the overlay's id-specificity `display:block !important` beats any stray `body > *` rule — so the original blank-page cause can't recur. Updated [tests/security-info-page.test.js](tests/security-info-page.test.js): the print-isolation test now asserts siblings are `display:none` (not merely `visibility:hidden` — the no-blank-pages guarantee) and the overlay is `position:static` (not absolute/fixed). CSS-only + test; full suite 73/73, 2647 tests; `audit-docs` clean.
- **fix(compliance): CGL dashboard stuck on "Loading…" until a manual Refresh** (May 19, 2026):
  On first open the data loaded (418 policies, 101 rows in `#cglTableBody`, correct footer) but the UI never swapped: `#cglLoading` stayed `display:block` and `#cglTableContainer` stayed `display:none` until the user clicked **Refresh**. Root cause is a **visibility-ownership gap**, not a gated swap (`_showCachedData`'s swap is unconditional and before render): the boot-time Supabase cloud-restore emits an ActivityLog `{type:'sync'}` event → debounced `_applySyncedReload()` ([js/compliance-dashboard.js](js/compliance-dashboard.js)), which re-reads state and repaints rows **but never revealed the table**; its re-entrant no-cache `fetchPolicies()` (`else` branch) can also set `loading=block / table=none`, so rows ended up rendered into a hidden table. **Refresh "fixed" it** only because `forceRefresh()` routes through `fetchPoliciesFromAPI()`, whose success path does the swap. Fix: a single idempotent **`_revealTable()`** (hide loading, show table, clear error) is now the one source of truth, called by the implicated path (`_applySyncedReload`'s rows-exist branch — the underlying fix), at the **end** of `_showCachedData` as a defensive guarantee (so `init()`'s `await fetchPolicies()` always resolves with the table visible even under the boot race), and the early `_showCachedData` reveal was kept; `deduplicateRenewals()/checkForRenewals()/verifyAllClients()` are now wrapped in try/catch so a throw there can't strand the overlay. No-cache / API-fetch / `showLocalDevMode` / Refresh paths are unchanged (no regression). Extended [tests/cgl-sync-reload.test.js](tests/cgl-sync-reload.test.js) +4 (idempotent `_revealTable`; `_showCachedData` reveals on cache hit and even when a renewal step throws; the boot sync event reveals the table with zero API calls). Comments kept terse so `compliance-dashboard.js` stays **under its frozen 3,040-line `audit-docs` ceiling** (3,038) — no ceiling raise, no disproportionate split for a bugfix. Full suite 73/73, 2647 tests; `audit-docs` clean.
- **fix(security): Security page "Print / Save PDF" produced a blank page** (May 19, 2026):
  The print/export came out as one big blank page. Two compounding causes. **(1) An unscoped foreign print rule.** [css/task-sheet.css](css/task-sheet.css) ships a plain (non-body-class-scoped) `@media print` block — `body > *:not(.app-shell){display:none!important}` — that loads *after* [css/security-info.css](css/security-info.css) and therefore applies to **every** print in the app, hiding `#securityInfoOverlay` (a body child that isn't `.app-shell`). Excluded the overlay at the source (`:not(.app-shell):not(#securityInfoOverlay)`, one token, can't regress task-sheet which never shows that overlay) **and** rebuilt the security print isolation on the bulletproof **visibility** pattern (`body.secinfo-printing * { visibility:hidden }` + re-show only the overlay subtree) instead of the brittle `display:none`-siblings approach that lost the specificity fight — `visibility` can't be collapsed away by a foreign `display:none`, and an id-specific `display:block !important` on the overlay still beats the task-sheet rule so it stays rendered. **(2) A premature cleanup timer.** `SecurityInfo.print()` ([js/app-boot.js](js/app-boot.js)) removed `body.secinfo-printing` after a 2 s `setTimeout` — but "Save as PDF" routinely takes longer than 2 s, so the print CSS was stripped while the dialog was still open and the page rasterized blank. Dropped the timer entirely; cleanup is now `afterprint`-driven (reliable across modern browsers) with a window-`focus` backstop that can only fire once the dialog closes. Extended [tests/security-info-page.test.js](tests/security-info-page.test.js) (visibility-based isolation asserted; the brittle `body > *` pattern banned from the block; the no-premature-timer contract; the task-sheet exclusion). Full suite 73/73, 2643 tests; `audit-docs` clean.
- **fix(security): correct contact email, drop em dashes, tighten three claims to 100% accuracy** (May 19, 2026):
  Owner review of the new Security & Data Protection page. **(1)** Contact corrected `austin@altech.agency` → **`austin@altechinsurance.com`** (mailto + visible text). **(2)** All em dashes removed from the page copy (rephrased to commas / colons / parentheses); unrelated em dashes elsewhere in [index.html](index.html) were intentionally left untouched. **(3) Three claims re-verified against source and tightened so an auditor can't fault them:** (a) "Every record … written to storage or sent to the cloud" overclaimed — not every local key is encrypted (the activity log etc. are plaintext-local); reworded to "the client information you enter is encrypted with AES-256-GCM before it is synced to the cloud, and sensitive data is encrypted at rest on this device." (b) "the unwrapped key … is never written to disk" was inaccurate — [js/crypto-helper.js](js/crypto-helper.js) `_saveSessionMK()` mirrors the master key into **`sessionStorage`** so an F5 doesn't re-prompt; reworded to state it precisely (held in memory, mirrored only into the tab's session storage, bound to the signed-in account, never in persistent local storage, cleared on tab/window close). (c) CSP was shown truncated as `default-src 'self'` with "blocks injected third-party code" — the real header is `default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline'`; now shows the **exact** value (an auditor copy-checking the response header would have caught a truncation that hides `'unsafe-inline'`) with a neutral description. Extended [tests/security-info-page.test.js](tests/security-info-page.test.js) (+2 tests: correct email / no old domain; zero em dashes) and bounded the test's overlay slice precisely to the modal so unrelated copy can't mask a regression. Suite 73 (extended, no new file); `audit-docs` clean.
- **feat(security): auditor-facing "Security & Data Protection" page — accurate, in-app, printable** (May 19, 2026):
  Agencies need to show an auditor how client data is stored/encrypted. An in-app `#securityInfoOverlay` modal already existed but was **stale and false** — it claimed *Firebase Authentication*, *"Multi-tenant Firestore rules"*, and *TLS 1.3* (Firebase was removed May 2026; Supabase is the sole backend; the app does not control the TLS version). An audit page asserting controls that don't exist is worse than none. Rewrote it from the **actual source** (verified line-by-line, not from docs): a plain-language summary (encrypted-at-rest, only-you-can-unlock, per-account isolation, strong sign-in, offline/local, hardened connections) plus a collapsible **"Technical detail (for IT / auditors)"** section with the real controls — AES-256-GCM + 96-bit IV + AAD-bound v2 envelopes; per-user 256-bit master key wrapped twice (passphrase + recovery), never sent to the server; **Argon2id** (64 MiB/t3/p1) for new vaults, PBKDF2-SHA-256 600k legacy / 100k legacy device key; HKDF-SHA-256 domain separation; **Postgres Row-Level Security** with the self-checking audit migration + operator verify script; **Supabase Auth** with server-side token verification + **TOTP and WebAuthn-passkey MFA**; the exact code-enforced security headers (HSTS `max-age=31536000; includeSubDomains`, CSP `default-src 'self'`, X-Frame-Options DENY, etc.); local-only/never-synced secrets; client-side-only exports. Includes an honest scope disclaimer (*"describes the build; not an independent audit or certification"* — no SOC 2/ISO/HIPAA/PCI claims). Reachable via a new **padlock button in the sidebar footer** between the night-mode and account buttons ([js/dashboard-widgets.js](js/dashboard-widgets.js), `icon('lock')` → `SecurityInfo.open()`). [js/app-boot.js](js/app-boot.js) `SecurityInfo` gained `print()` — opens the page, force-expands the technical `<details>`, scopes printing via `body.secinfo-printing`, restores after `afterprint`; a **🖨️ Print / Save PDF** button drives it. [css/security-info.css](css/security-info.css): wider modal (520→720), toolbar, collapsible-section + `code`/`h5` styles, dark-mode, and a print-scoped `@media print` block (toner-friendly black-on-white, no app chrome, gated entirely on `body.secinfo-printing` so an unrelated `window.print()` is unaffected). New regression suite [tests/security-info-page.test.js](tests/security-info-page.test.js) (10 tests) fails the build if the false Firebase/Firestore/TLS-version claims ever return, if a real control disappears, if a certification is implied, or if the padlock/print wiring breaks. Suite 72→73; `audit-docs` reconciled.
- **fix(compliance): the morning CGL cache warm now actually reaches the dashboard — stop the "spins every morning" cold-fetch** (May 19, 2026):
  The May 18 "daily morning cron warms the CGL cache" feature was inert in production: the dashboard still spun on "Connecting to HawkSoft…" every morning across devices. Four compounding bugs, all fixed: **(1) Key-namespace mismatch (root cause)** — [api/compliance.js](api/compliance.js) (cron + server) reads/writes the BARE Redis key `cgl_cache`, but [api/kv-store.js](api/kv-store.js) (the only KV path the browser uses) UID-prefixed *every* key (`uid:<id>:cgl_cache`), so the cron warmed a key no client could read (and nothing ever wrote the per-user one — `_syncToKV` is only ever called with `cgl_state`). `cgl_cache` is now a SHARED key (`SHARED_KEYS = ['cgl_cache']` → `redisKey()` helper); `cgl_state`/`email_drafts`/`export_history` stay per-user. Safe: endpoint stays `requireAuth`-gated and `cgl_cache` holds whole-agency policy data every user already sees (keyed by `HAWKSOFT_AGENCY_ID`), no per-user secrets. **(2) Self-defeating server TTL** — `KV_CACHE_TTL_MS` was 15 min, so a once-daily warm was "too stale to serve" by 08:15 and every later open cold-refetched; raised to 24 h (the cron's `?refresh=true` still force-pulls fresh each morning; the client does its own 15-min staleness + silent background refresh). **(3) Cron timing** — `0 15 * * *` (= 08:00 Pacific, *arrival time*) → `0 13 * * *` (= 06:00 Pacific, ~2 h before the workday; matches `reminders-sweep`) in [vercel.json](vercel.json). **(4) Client abort too early** — `API_TIMEOUT_MS` 65 s (stale "Vercel limit is 60s" — `vercel.json` grants `api/compliance.js` `maxDuration: 300`) → 120 s in [js/compliance-dashboard.js](js/compliance-dashboard.js), so a legitimate cold full-agency pull finishes and warms the shared cache (next open / other devices instant) instead of dropping the user on "No Data". Also made the >1 MB KV-write skip in [api/compliance.js](api/compliance.js) log loudly instead of being a silent no-op `else`, so a recurrence is diagnosable from Vercel logs. New regression suite [tests/cgl-cache-cron-warm.test.js](tests/cgl-cache-cron-warm.test.js) (8 tests) pins all four fixes + the cron/kv key agreement; existing `kv-store-cache-miss` / `api-compliance` suites stay green. `audit-docs` clean.
- **feat(hawksoft): agent initials are a per-user, cloud-synced setting — the Path A `<I>`→`<AJK>` interim** (May 19, 2026):
  HawkSoft attributes every partner-API log to the generic integration user (`<I>`) because `integration.hawksoft.app` authenticates with agency-level Basic-auth credentials and the request body has no `created_by` field ([api/hawksoft-logger.js](api/hawksoft-logger.js)). True per-user attribution needs the unsanctioned internal `cs.hawksoft.app` ("Path B") via the Chrome extension — greenfield, gray-area TOS, and blocked on live-session captures. Shipped the sanctioned, zero-TOS-risk interim instead: the agent-initials prefix that already provides `RE:`-line attribution is no longer a manual, per-device, blank-able field. New `STORAGE_KEYS.HAWKSOFT_INITIALS` ([js/storage-keys.js](js/storage-keys.js)) synced per-user via `DOC_LOCAL_KEYS.hawksoftInitials` ([js/supabase-sync.js](js/supabase-sync.js)) — the documented one-line "add a sync type". Captured once in onboarding step 2 ([index.html](index.html), [js/onboarding.js](js/onboarding.js) `_saveInitials`/`getInitials`) and editable later under **Settings → Preferences → 🪪 HawkSoft Initials** ([index.html](index.html), loaded in [js/auth.js](js/auth.js)). [js/call-logger.js](js/call-logger.js) `_load()` now resolves initials **form-value-first → synced setting** (prefilled so it's never retyped on a new device) and `_persistInitialsOnly()` mirrors a correction back into the setting so it propagates everywhere. Still **verbatim user input — never derived from the display name** (middle initials: AJK ≠ AK would put the wrong ID in shared logs). HawkSoft still *owns* the log as the integration user — this makes the visible attribution reliable, not true; the full Path B execution plan (hard live-capture prerequisites + the verified `chrome-extension-v3` state + the sanctioned-rep escape hatch) is recorded in [docs/HAWKSOFT_API.md](docs/HAWKSOFT_API.md). Updated `tests/call-logger.test.js` for the new resolution contract + the mirror; suite 70→71; `lint-aad` + `audit-docs` clean.
- **feat(hawksoft): AI-marked emphasis + in-flow rendered preview for the logger** (May 19, 2026):
  Follow-up to the formatted-logs wiring. **(1) Smarter bolding** — the format prompt ([api/hawksoft-logger.js](api/hawksoft-logger.js)) now allows the *only* markup `**double asterisks**` and instructs the AI to emphasize the few things that matter (quote/policy numbers, dollar amounts, carrier names, dates/deadlines, the key action), never whole sentences, and **never on the RE: line**. The converter ([js/call-logger.js](js/call-logger.js)) gained `_parseEmphasis()` — paired `**…**` → bold runs, stray/unpaired `**` stripped so literal asterisks never reach HawkSoft, line 1 strips `**` and stays span-free (collapsed-list-safe). `_runsForBody()` parses emphasis first and only falls back to the deterministic `Action:` auto-bold when the AI marked nothing (no double-processing); lists get emphasis too. Agents can also hand-type `**…**` in Edit for manual control. All emphasized text still flows through `buildHawkSoftNote()`'s escape boundary. **(2) See-before-you-send** — a Plain ⇄ Formatted toggle in the FORMATTED LOG header ([plugins/call-logger.html](plugins/call-logger.html)) renders the *exact bytes* Confirm & Push will send (via `_setPreviewView()` → `_plainToHawkSoftHtml`, read-only, builder-sanitized) on HawkSoft's white surface; entering Edit forces back to Plain. Scoped styles in [css/call-logger.css](css/call-logger.css). Extended [tests/call-logger-html-format.test.js](tests/call-logger-html-format.test.js) (emphasis parsing, RE-line strip, AI-marked vs legacy `Action:`, list emphasis, escaping, the preview toggle incl. edit-mode lockout); updated the now-intentionally-changed prompt assertion in [tests/hawksoft-logger.test.js](tests/hawksoft-logger.test.js). Attribution unchanged — `<I>`→`<AJK>` still needs Path B (documented). Suites 70 (extended, no new file); `audit-docs` clean.
- **feat(hawksoft): formatted logs via the SANCTIONED partner API — verified, then wired in** (May 18, 2026):
  The HawkSoft 6 exploration guide (and our own AI prompt + `HAWKSOFT_API.md`) asserted the partner API is *plain-text only* and that rich formatting needed the unsanctioned Path B / Chrome-extension route. **Disproven in production:** a `buildHawkSoftNote()` payload pushed through the existing sanctioned partner API ([api/hawksoft-logger.js](api/hawksoft-logger.js) → `integration.hawksoft.app`, the logger's own "Confirm & Push") rendered correctly in HawkSoft — bold, numbered list, `rgb()` color, underline, and the Phone › From › Insured channel all came through (log Row #234). So formatting needs **no extension, no gray-area TOS** — the path we already ship does it. Wired it in: new deterministic `_plainToHawkSoftHtml()` / `_plainToNoteContent()` in [js/call-logger.js](js/call-logger.js) convert the reviewed plain text → the verified `buildHawkSoftNote()` dialect on Confirm & Push, pushed through the **same** partner API (zero server change). **List-view-safe by construction**: line 1 stays a plain `<div>` (no spans) so HawkSoft's collapsed log-list shows a clean `RE: AJK — summary`; the body gets reliable paragraph breaks, a bolded `Action:` label, and real `<ul>/<ol>` for bulleted/numbered lines. Still gated behind the existing human Edit/Confirm; `_pendingLog.formattedLog` stays plain (retry/diagnostics/manual-copy); safe fallback to plain text if the builder is unavailable/empty (a push is never blocked). The AI deliberately keeps emitting plain text (reliable) — richer AI-driven emphasis/color is a tracked follow-up. New [tests/call-logger-html-format.test.js](tests/call-logger-html-format.test.js) locks the converter (first-line-plain contract, `Action:` bold, list collapse, blank→break, XSS escaping via the builder, empty→null, builder-missing fallback, determinism) + source-asserts the Confirm wiring & plain fallback; the now-intentionally-changed Step-2 assertion in [tests/call-logger.test.js](tests/call-logger.test.js) was updated to lock the new contract. [docs/HAWKSOFT_API.md](docs/HAWKSOFT_API.md) + [docs/hawksoft-dictionary.json](docs/hawksoft-dictionary.json) updated with the VERIFIED finding (Path A renders HTML; Path B demoted to an attribution-only follow-up since Path A still logs as the generic integration user). Suite 69→70; `audit-docs` clean.
- **feat(hawksoft): rich-format preview (DRY RUN) in the HawkSoft Logger** (May 18, 2026):
  Added a collapsed, clearly-badged "Rich-format preview — nothing is sent to HawkSoft" panel at the bottom of the HawkSoft Logger ([plugins/call-logger.html](plugins/call-logger.html)) so the new `buildHawkSoftNote()` dialect can be *seen* before any Path B exists — without risking the live logger. Preset buttons (bold/italic, list, link+color, mixed call log) fill an editable JSON-IR textarea; **Generate** shows the exact HawkSoft HTML that would be sent (as escaped source) **and** a live rendered preview on a white surface mirroring HawkSoft's log view. The panel is **deliberately isolated**: new pure `CallLogger._rpBuild(jsonStr)` only does `JSON.parse` → `window.HawkSoftNote.buildHawkSoftNote` (no DOM, no network), and `_initRichPreview()` wires only `clRp*` ids behind an idempotent guard — it never references `apiFetch`/`_pendingLog`/`_handleConfirm`, shares no state with the push path, and `renderOut.innerHTML` is assigned **only** builder output (the sanitization boundary: text escaped, link schemes allow-listed, fixed tag set). The live partner-API logger above is byte-for-byte unchanged and still plain-text. New [tests/call-logger-rich-preview.test.js](tests/call-logger-rich-preview.test.js) asserts both correctness (each preset → expected HTML; `javascript:` link degrades to inert text; invalid JSON surfaces an error without throwing) and the isolation guarantees (source analysis: no push/network refs in the preview functions; behavioral: preview never calls `Auth.apiFetch`). Scoped styles in [css/call-logger.css](css/call-logger.css) (design tokens; intentional light render surface documented). Suite 67→68; `audit-docs` clean.
- **feat(hawksoft): `buildHawkSoftNote` IR→HTML builder + `HawkSoftAPIError` + API dictionary/reference** (May 18, 2026):
  A DevTools exploration of HawkSoft 6 surfaced a more capable internal API (`cs.hawksoft.app`, "Path B") that accepts HTML notes + real user attribution — things the sanctioned partner-API logger ([api/hawksoft-logger.js](api/hawksoft-logger.js)) can't do (plain-text, generic-user, worked around by prepending agent initials to the `RE:` line). Path B needs a live HawkSoft session + designated test client to verify and is **deliberately not wired** this round. Instead, landed the **path-agnostic, fully unit-tested foundation** the exploration itself lists as a prerequisite: new pure module [js/hawksoft-note.js](js/hawksoft-note.js) (`window.HawkSoftNote`, mirrors the `hawksoft-renderers.js`/`crypto-aad.js` pattern, loaded before `hawksoft-renderers.js` in [index.html](index.html)) exposing **`buildHawkSoftNote(content)`** — converts a structured intermediate representation (paragraph/bullet/numbered/break blocks; text/link runs) into HawkSoft's exact contentEditable HTML dialect (`<span style="font-weight: bold;">`, `<div>`/`<div><br></div>`, `<ul>/<ol>`, `<a target="_blank">`), so only this one function changes if HawkSoft's format does. It is a **security boundary**: all text HTML-escaped via `Utils.escapeHTML`, link `href` control-stripped + scheme-allowlisted (`http`/`https`/`mailto` only; `javascript:`/`data:`/`vbscript:`/relative degrade to inert escaped text) + attr-escaped, rgb components validated/rounded/clamped `[0,255]`, unknown/typeless blocks & runs skipped. Plus **`HawkSoftAPIError(status, body)`** with guarded `isAuthError`/`isNotFound`/`isServerError`. New [tests/hawksoft-note.test.js](tests/hawksoft-note.test.js) (~70 cases, mirrors the `cgl-utils.test.js` JSDOM harness) covers every formatting variant, combined-span determinism, color clamping, XSS/scheme rejection, degenerate inputs, and the error truth table. Also committed [docs/HAWKSOFT_API.md](docs/HAWKSOFT_API.md) (Path A vs B reference + roadmap; **corrects the exploration guide's outdated claim that Path A "cannot set activity channels"** — the partner API already sets channel/method/direction/party via `CHANNEL_MAP`) and [docs/hawksoft-dictionary.json](docs/hawksoft-dictionary.json) (Path A code-mirror + Path B confirmed-vs-`TBD`, nothing guessed), indexed in [docs/DOCUMENTATION_INDEX.md](docs/DOCUMENTATION_INDEX.md). No behavior change to the live logger by design. Suite 66→67, `audit-docs` clean (AGENTS module count 108→109, JS total + grouped-list refreshed).
- **fix(intake-v2): EZLynx import safety — never emit XML that silently fails the import** (May 18, 2026):
  A real failing export (rated co-applicant with no DL #) prompted the rule: *the agent should see a clear "your export is missing X" error, never a silent EZLynx import failure.* Hardened the native EZAUTO/EZHOME builders ([js/intake-v2-export-ezlynx.js](js/intake-v2-export-ezlynx.js)) so a structurally-doomed file can't be produced: **(1) license-less rated driver** — a `<Driver>` with `<Rated>Rated</Rated>` but no `<DLNumber>` (e.g. a synced co-applicant whose DL was never captured — exactly the field the per-keystroke focus bug made hard to type) made EZLynx reject the import; it now degrades to `<Rated>Not Rated</Rated>` so the file imports. **(2) Invalid VIN** — `UseVinLookup` was set to `Yes` whenever *any* VIN string existed, so a short/garbled VIN made EZLynx's decoder fail the whole import; now `UseVinLookup=Yes` + `<Vin>` are emitted only for a structurally valid 17-char VIN (no I/O/Q), otherwise `UseVinLookup=No` and EZLynx rates from Year/Make/Model. **(3) Junk gender** — `expandGender` passed unknown values (`Prefer not to say`, blank) straight through, producing an invalid V200 `<Gender>` enum; it now always resolves to `Male`/`Female`/`Not Specified`. **Pre-export validator** `validateIntakeV2ForEZLynx()` runs in the orchestrator ([js/intake-v2-export.js](js/intake-v2-export.js)) before any file is written: **blocking** issues (no named insured, incomplete address, no drivers, an unidentifiable vehicle) abort the export with a visible list to fix; **warnings** (a driver exported Not Rated for want of a DL#, a VIN dropped, no liability/Cov-A) show the agent the exact missing fields and let them export anyway (the file is hardened to still import). New suite **`IntakeV2 — EZLynx import safety`** in [tests/intake-v2.test.js](tests/intake-v2.test.js) (4 tests) locks the Not-Rated / VIN-lookup / gender degradation and the validator's blocking/warning output; existing CMSMTF/EZLynx wrapper + parity suites stay green (no assertion regression). Full suite **66/67, 2502 passing** (lone failure the pre-existing unrelated `intake-v2` Progressive-bindability test); `lint-aad` + `audit-docs` clean.
- **feat(intake-v2): V1→V2 export parity — effective date, policy term, prior status, earthquake** (May 18, 2026):
  A V2-vs-V1 audit (field registry + both native exporters) found V2 is a *superset* of V1 except for a few real gaps that blocked "both fully compatible with both export options." Fixed the HIGH ones so V2 can run alongside V1: **(1) Effective date was being lost** — [js/intake-v2-export-cmsmtf.js](js/intake-v2-export-cmsmtf.js) hardcoded `gen_tEffectiveDate = (today)` and [js/intake-v2-export-ezlynx.js](js/intake-v2-export-ezlynx.js) EZAUTO emitted no `<Effective>`, silently discarding the agent's requested bind date. Added `priorInsurance.effectiveDate` (+ `policyTerm`, `priorPolicyStatus`) to the v2 schema ([js/intake-v2-fields.js](js/intake-v2-fields.js)) with defaults in [js/intake-v2-init.js](js/intake-v2-init.js); CMSMTF now emits the real `_dateHS` date (falling back to `(today)` only when blank) + `gen_nTerm`; both EZAUTO and EZHOME `<PolicyInfo>` now carry `<Effective>` + a real `<PolicyTerm>`. **(2) Earthquake had no v2 input at all** and V2 EZHOME omitted the `<Earthquake>` block V1 emitted (CEA / earthquake quoting silently lost the signal on *both* exports). Added per-home `coverages.earthquake` / `earthquakeDeductible` / `earthquakeZone` ([js/intake-v2-fields.js](js/intake-v2-fields.js) + `itemDefaults('homes')` in [js/intake-v2-core.js](js/intake-v2-core.js)); CMSMTF now emits `gen_bEarthquake` / `gen_sEQDeduct` / `hpm_sEarthquakeZone` (V1 tag parity) + an FSC note line; EZHOME emits the nested `<Earthquake>` block plus `endorsements_earthquake_*` ExtendedInfo valuepairs. **(3) Prior policy status** (`Non-Renewed` / `Cancelled` — a carrier underwriting signal) now flows to CMSMTF misc. **Test coverage gap closed:** every other ezlynx/cmsmtf contract test exercises only the *V1* builders — the V2 native CMSMTF/EZLynx builders had **zero** parity coverage, so these divergences were unguarded. New suite **`IntakeV2 — export parity (effective date, term, earthquake)`** in [tests/intake-v2.test.js](tests/intake-v2.test.js) (4 tests) drives the v2 builders and asserts the real date / term / earthquake survive into both formats. MEDIUM gaps (flood coverage, systems decline-signals, full endorsement set, split alarms, jewelry/UMPD limits) tracked for a follow-up — V1 remains available as the complete path. Full suite **66/67, 2498 passing** (lone failure the pre-existing unrelated `intake-v2` Progressive-bindability test); `lint-aad` + `audit-docs` clean.
- **fix(intake-v2): operator-card fields lost the caret after every character** (May 18, 2026):
  Typing in any Personal Intake **v2** operator/driver card field (reported on **DL Number**, but every operator field was affected) lost cursor focus after each keystroke. Root cause: the delegated `input` handler in [js/intake-v2-core.js](js/intake-v2-core.js) fired `requestRerender('operators')` on **every keystroke** for the operators collection branch (`if (collKey === 'operators')` — field-agnostic), and the operators renderer ([js/intake-v2-operators.js](js/intake-v2-operators.js) `render()`) does a blunt `root.innerHTML = …` swap, destroying and recreating the very `<input>` being typed in. The scalar branch directly above already documented and guarded against exactly this (only re-syncs operators for the handful of `OP_SYNC_PATHS` applicant fields, explicitly "to avoid re-rendering the operator pool on every keystroke") — the collection branch never got the same guard. **Two-part fix:** (1) gate the operator fan-out on `e.type === 'change'` (commit/blur) instead of `input` — `setItemField()` still runs on every `input` so data / export / bindability stay correct per keystroke, `<select>`/checkbox operator fields still update instantly (their `change` fires immediately), text fields' dependent surfaces (card title, age, status dot, product-card chip names) update when focus leaves; (2) made `render()` focus/caret-preserving — it captures `document.activeElement` + selection range when the active element is inside the operators root and restores them after the innerHTML swap (deterministic `iv2-${idStem}-${itemId}` ids survive the rebuild), a safety net for the legitimate re-renders that remain (tabbing between operator fields, the synced-applicant path that re-renders the pool while you type in Quick Start). New regression suite **`IntakeV2 — operator field focus stability`** in [tests/intake-v2.test.js](tests/intake-v2.test.js) (3 tests). `lint-aad` + `audit-docs` clean.
- **feat(prospect): higher hit-rate business search — AI fallback, relaxed name matching, WA DOR source** (May 18, 2026):
  "Search a business and it doesn't find much" had five compounding causes; this pass fixes all of them. **(A) No more dead-end.** Phase-1 discovery (`search()` in [js/prospect.js](js/prospect.js)) used to query only L&I + SOS + Google Places and, on zero candidates, `_toast('No businesses found')` then `return` — stranding the user even though the AI dossier (Gemini + Google Search grounding) can research almost any business by name+city+state. The single most powerful tool was unreachable precisely when it was needed most. Zero candidates now falls straight through to `investigateManual()` (full investigation → AI). **(B) Relaxed name matching.** New [api/_prospect-name-match.js](api/_prospect-name-match.js) (`normalizeBusinessTokens` / `buildSoqlNameClause` / `relaxedSearchValue`) strips entity-suffix and connector noise (LLC/INC/CORP/CO/LP/LLP/LTD/PLLC/PC/&/AND, leading "THE") and builds a `upper(col) like upper('%a%b%')` clause (tokens joined by `%` so any registered suffix and interior words still match, order preserved, SoQL-injection-safe). Wired into WA L&I, OR CCB, OR SOS (Socrata `$where`), WA SOS CCFS (`SearchValue`/Contains) and WA DOR — so "ABC Plumbing LLC" now finds "ABC PLUMBING INC". Falls back to the original raw-substring query when normalization yields no tokens (never emits a bare `%%` full-table scan). **(C) WA DOR is now a first-class Phase-1 source.** `tryWADORLookup` (every WA business with a tax registration — restaurants, retail, services, medical, not just licensed contractors) was previously reachable only as a CCFS-blocked fallback; exposed via new `handleDORLookup` + `?type=dor` route ([api/prospect-lookup.js](api/prospect-lookup.js)) and queried in parallel during discovery (WA-only, no-op elsewhere), filling the biggest non-contractor coverage gap. **(D) Source-gap transparency.** `ProspectFormatters.sourceGapNotes()` surfaces a muted advisory in the selection UI when Google Places is unconfigured (`PLACES_API_KEY`) or SOS was anti-bot-blocked — a thin result is now explained, not silent. **(E) City disambiguation.** The City field (previously ignored by L&I/SOS) now feeds the AI prompt + `buildDataContext` ([api/_prospect-ai-analysis.js](api/_prospect-ai-analysis.js)) and ranks candidates (state-match → city-match → corroborating-source count). **Decomposition note:** `prospect.js` is size-ceilinged (frozen 1,975 in [scripts/audit-docs.js](scripts/audit-docs.js)); rather than grow it, the pure candidate-collection/dedupe/gap-note logic moved into the existing [js/prospect-formatters.js](js/prospect-formatters.js) sibling (`collectCandidates` / `dedupeCandidates` / `sourceGapNotes` / `gapNoteBanner`) — `prospect.js` net **1,950 → 1,850**. New suite [tests/api-prospect-name-match.test.js](tests/api-prospect-name-match.test.js) + rewritten behavioral candidate tests in [tests/prospect-client.test.js](tests/prospect-client.test.js) (now exercise the relocated helpers + DOR + city ranking instead of source-string grepping). `lint-aad` clean; `audit-docs` clean (suite count 66→67).
- **feat(theme): clean Light theme (white & blue) — Settings → Preferences** (May 18, 2026):
  The base `:root` light palette is a *warm cream* look (`--bg:#FAF7F4`, tan borders, orange accent gradients), so "just turn dark mode off" never matched the cool white/blue of agency desktop software (HawkSoft / ForAgentsOnly / Progressive). Added a dedicated **`light`** theme: new [css/theme-light.css](css/theme-light.css) overrides the full design-token set to a crisp cool palette — Apple blue `#007AFF` accent, soft off-white app bg `#F4F7FB`, pure-white cards with subtle borders/shadows, cool slate text — plus targeted polish (card elevation, white sidebar/header, kills the dark-tuned ambient/gradient backdrops, light scrollbar). Wired exactly like Aurora ([js/app-ui-utils.js](js/app-ui-utils.js)): added to `_VALID_THEMES` (`'default'|'aurora'|'light'`), a Theme-dropdown option ([index.html](index.html)), and a `setTheme('light')` branch that is the **inverse of Aurora** — it forces dark mode OFF (removes `body.dark-mode`, persists `DARK_MODE='false'`, updates the icons + PWA `theme-color`); `toggleDarkMode()` gained the symmetric guard (toggling dark back on while Light is active → `default`). `loadTheme()` already replays non-default themes on boot, and THEME/DARK_MODE are already cloud-synced via `schedulePush`. Default stays unchanged — Light is opt-in. Full suite **66/67, 2491** green (lone failure pre-existing unrelated `intake-v2`); `audit-docs` clean (AGENTS.md CSS counts + a new Light Theme doc section).
- **feat(compliance): daily morning cron warms the CGL cache — fresh by start of day, no manual Refresh** (May 18, 2026):
  The CGL/Commercial dashboard only pulled fresh HawkSoft data when a client opened the page and the cache was stale — so the first person in each morning either saw yesterday's data or sat through a 30–60 s cold HawkSoft pull, and often clicked **Refresh** to force it. Added a second Vercel cron in [vercel.json](vercel.json) — `{"path": "/api/compliance?refresh=true", "schedule": "0 15 * * *"}` — alongside the existing `reminders-sweep`. `?refresh=true` is an already-supported, side-effect-only path (the client's own background refresh uses it — [js/compliance-dashboard.js](js/compliance-dashboard.js):1070): it bypasses the 15-min Redis read, force-pulls HawkSoft, and **re-writes the shared `cgl_cache` KV** for the next request. `/api/compliance` is `securityMiddleware`-wrapped (rate-limit/CORS only — *not* `requireAuth`, and the client already calls it unauthenticated), so the cron call passes with no new endpoint, no `CRON_SECRET` wiring, and no code. The compliance function already has `maxDuration: 300` (ample for a 30–60 s pull). Net effect: the shared cache is freshly populated every morning before the Pacific workday, so the existing client-side background refresh on dashboard-open serves fast, current data with no manual click. **Schedule note:** `0 15 * * *` is 15:00 UTC = **08:00 PDT / 07:00 PST** — chosen so data is always fresh *by* ~8 am Pacific year-round (earlier in winter is harmless; later would miss the ask). Vercel cron is UTC and does not follow DST; bump to `0 16 * * *` for strict 08:00-PST-in-winter if ever desired.
- **test(compliance): dedicated unit suites for the extracted `cgl-utils` / `cgl-renderers` modules** (May 15, 2026):
  After the two decomposition cuts, `js/cgl-utils.js` (15 pure helpers) and `js/cgl-renderers.js` (3 pure HTML builders) were covered only *indirectly* via the `compliance-dashboard.js` suites — the same gap the `hawksoft-renderers.js` precedent still has. New [tests/cgl-utils.test.js](tests/cgl-utils.test.js) and [tests/cgl-renderers.test.js](tests/cgl-renderers.test.js) (48 tests) exercise each function's contract directly: `escJsAttr` dual-escape ordering, `_smartMergeDict` non-destructive merge, `_migrateNote` legacy→log shape, `formatNoteTime` buckets, note icon/label mapping, `_scrubUndefined` undefined/empty-key drop + recursion, `getStatusLabel` boundaries, `clientLink` desktop/no-id branches, and every `liCcbBadges` / `classificationOverride` / `noteLog` branch incl. JS-escaped onclick payloads and newest-first ordering. Locks the modules' behavior so further extraction stays safe. Test-only; suite count 64 → 66 (docs updated).
...
### Changed
- **refactor(compliance): split disk/backup I/O into js/cgl-backup.js — compliance-dashboard.js back under its size ceiling** (May 19, 2026):
  [js/compliance-dashboard.js](js/compliance-dashboard.js) was **3274 lines vs its frozen `SIZE_GRANDFATHER` ceiling of 3275** ([scripts/audit-docs.js](scripts/audit-docs.js)) — zero headroom, so the *next* CGL change was blocked by the mandatory `audit-docs` pretest. Per the CLAUDE.md contract (never raise a ceiling — split + lower), extracted the cohesive desktop-disk + JSON/CSV backup-restore slice (`downloadFullBackup`, `restoreFullBackup`, `renderFileToolbar`, `updateFileStatus`, `openDiskFile`, `saveDiskFile`, `saveDiskFileAs`, `exportBackup`, `importBackup`) into new sibling [js/cgl-backup.js](js/cgl-backup.js) (`window.CglBackup`, IIFE), mirroring the `cgl-utils.js`/`cgl-renderers.js ← compliance-dashboard.js` precedent. These aren't pure (File System Access, FileReader, Blob downloads, `fileHandle`, localStorage) so they take the dashboard instance `d` and call back for core bits (`d._getStateSnapshot`/`d.saveState`/`d._safeLSWrite(STORAGE_KEYS.CGL_STATE,…)`/…); intra-slice calls stay direct. The dashboard keeps **one-line forwarders** so every `this.x()` call site and the two [plugins/compliance.html](plugins/compliance.html) `onclick`/`onchange` bindings work unchanged. Net **3274 → 3009** (−265); ceiling lowered **3275 → 3040** (downward-only, restores the original ~30-line maintenance margin); loaded after `cgl-renderers.js` in [index.html](index.html); [tests/cgl-sync-reload.test.js](tests/cgl-sync-reload.test.js) + [tests/compliance-state-preservation.test.js](tests/compliance-state-preservation.test.js) eval the new file; [tests/audit-cleanup-fixes.test.js](tests/audit-cleanup-fixes.test.js) `_safeLSWrite` assertion updated for the cross-file contract (3 in dashboard + the file-open one in cgl-backup.js). AGENTS.md JS module count 109→110; `audit-docs` clean.
- **docs(hawksoft, roadmap): reconcile HAWKSOFT_API "Next"; flag the stale roadmap docs** (May 19, 2026):
  [docs/HAWKSOFT_API.md](docs/HAWKSOFT_API.md) "Next — richer formatting" still listed AI bold/color emphasis as fully open even though #165 shipped `**…**` bold via `_parseEmphasis()` + the preview toggle — marked bold **done**, narrowed the remainder to **color** (small but unverifiable without a live HawkSoft push, so live-session-gated like the collapsed-list-view check). Separately, [docs/ROADMAP.md](docs/ROADMAP.md) (Feb 4 2026) and [docs/SAAS_ROADMAP.md](docs/SAAS_ROADMAP.md) (Feb 2026) **predate the May-2026 Phase-D Firebase removal** and still plan obsolete Firebase/Firestore/Firebase-Auth work — added a STALE banner to each so they stop misleading (Supabase is the sole backend; the live backlog is code + CHANGELOG + HAWKSOFT_API.md). Docs only.
- **ui(admin): tidy the User Management row layout** (May 18, 2026):
  Follow-up to #156/#157 — the row was messy: the `ADMIN`/`BLOCKED` badge was appended *inside* `.admin-user-email` (which has `overflow:hidden;text-overflow:ellipsis`), so a long email clipped the badge to "ADM"; and the self row showed two greyed, non-functional buttons. Restructured ([js/admin-panel.js](js/admin-panel.js) `_render`, [css/auth.css](css/auth.css)): email + badges now live in a flex `.admin-user-line` where only the email ellipses (`flex:0 1 auto;min-width:0`) and badges are `flex-shrink:0` pills (uppercased via CSS, never clipped); the display-name sub-line ellipses cleanly; actions are grouped in `.admin-user-actions`; the self row shows a quiet "You" chip instead of dead disabled buttons (server still enforces the self-block/self-de-admin guard). CSS-only + DOM-render; no API/contract change; `node --check` + `boot-loading`/`plugin-integration` (201) green; `audit-docs` clean.
- **feat(account): overhaul "Your Account" into a tabbed settings modal + resurrect dead controls** (May 18, 2026):
  The account modal was a narrow 400px stack of 7 `<details>` accordions mixing identity, app config, sync, and security — "fragmented and not user-friendly." Rebuilt ([index.html](index.html) account view) as a **6-tab modal** (Profile · Prefs · AI · Security · Team · Data) reusing the existing `.seg-group`/`.seg-btn`/`.seg-active` segmented control and the `settingsSlideIn` keyframe — every element id, inline handler, and the keep-local label's nested span/strong are preserved verbatim; the two `<details ontoggle>` glossary/EZLynx lazy-loads are re-implemented as a tab-activation hydrate in the new `Auth.acctTab()` (no-clobber guard kept). `_showView` toggles a `.auth-modal-wide` (460px) class on the account view only; `showModal` always (re)opens on Profile; `_refreshAdminVisibility` now also gates the Team tab (`#acctTabTeam`) and falls back to Profile if admin is revoked. ([js/auth.js](js/auth.js), [css/auth.css](css/auth.css))
  - **Audit discovery — `window.CloudSync` was entirely undefined** (Phase D removed it; [js/sync-facade.js](js/sync-facade.js) only ships `window.Sync`/`window.AuthFacade`). So Sync Now, the keep-data-local toggle, the glossary-save push, **and** Delete Cloud Data were all dead (`ReferenceError`, or a silent no-op behind ~10 plugins' `typeof CloudSync` guards) — not just the two named buttons. Added a small **`CloudSync` compatibility shim** in `sync-facade.js`: `fullSync`/`schedulePush`/`pushToCloud` delegate to the live `Sync` facade; new `setDisabled` persists `STORAGE_KEYS.CLOUD_SYNC_DISABLED` + toast; new `deleteCloudData` is implemented. The resurrected plugin `schedulePush` calls are debounced (3s) + idempotent + still gated by the existing admin-only `writeBlocked()` — minimal real added traffic, the originally-intended behavior.
  - **Delete Cloud Data** (typed `DELETE` confirm) erases the signed-in user's `user_blobs` + `user_quotes` rows via `SupabaseSync.deleteBlob`/`listQuotes`/`deleteQuote` **directly** — intentionally bypassing the admin-only `writeBlocked()` gate (self-erasure is a right-to-erasure action; RLS scopes every delete to the caller's own rows). `user_crypto_meta` is deliberately preserved (vault-meta owns it; deleting bricks E2E recovery); localStorage is untouched (cloud-only, matches the label); non-atomic so partial failures are reported and a re-run is idempotent. Reports via `App.toast` + `ActivityLog`.
  - **User Management** rebuilt as a new [js/admin-panel.js](js/admin-panel.js) IIFE (`window.AdminPanel`, loaded after `sync-facade.js`) + an `#adminPanelModal` reusing `.auth-modal*`. Admin-only; lists users and toggles `is_admin`/`is_blocked` via the existing `api/admin-supabase.js` (`?action=list|update`) through `Auth.apiFetch`; rows are DOM-built with `textContent` (XSS-safe); self-row destructive buttons disabled to mirror the server's self-protection guards. Pure IIFE (no load-time DOM) so it's safe under `tests/load-html.cjs`.
  - `.settings-section*`/`.settings-subsection`/`.settings-select`/`.glossary-*` CSS retained (still used by `aurora-theme.css` + the moved controls + the AI Advanced `<details>`); a follow-up dead-CSS sweep can remove the now-unused accordion shell. Known pre-existing & out of scope: `Sync.refreshUI` is a no-op (`SupabaseSync` has none) so the sync-status line stays static "Ready to sync". Full suite **66/67, 2491 tests** green (lone failure is the pre-existing, unrelated `intake-v2` Progressive-bindability test); `npm run audit-docs` clean (AGENTS.md counts bumped for the new module).
- **ui(quickref): shrink the oversized Quick Emoji cards** (May 18, 2026):
  Follow-up to the Quick Ref pass — the emoji buttons were locked to `height: 92px` with `padding: 14px 8px 12px` ([css/quickref.css](css/quickref.css) `.qr-emoji-btn`) but only hold ~42 px of content (26 px icon + gap + 11 px label), so every card carried ~50 px of dead vertical space (visible in the deployed screenshot). Tightened to `height: 64px` + `padding: 10px 8px` + `gap: 4px` — ~28 px shorter per row, still a comfortable 64 px touch target (well above the 44 px min). The dashed "Add" button (`.qr-emoji-add-btn`, shown in edit mode) inherits `.qr-emoji-btn` so it stays in sync. CSS-only; no test pins emoji dimensions.
- **ui(quickref): Quick Emojis above Spell It Out; collapse the wasted space under the speller** (May 18, 2026):
  Two Quick Reference layout tweaks per user request. (1) **Reorder** ([plugins/quickref.html](plugins/quickref.html)) — the `#qrs-emojis` card now renders **before** `#qrs-speller` (Quick Emojis is the more frequently used surface); section comments renumbered ①Quick Emojis → ②Spell It Out → ③Agent ID Cards → ④Quick Dial → ⑤APCO. Pure block reorder — both are self-contained `.card.qr-section` divs keyed by `id`, `toggleSection()` is id-based, no JS/test depends on DOM order (div balance verified 42/42). (2) **Tighten the Spell It Out card** ([css/quickref.css](css/quickref.css)) — `#qrSpellerOutput` is `innerHTML=''` until the user types (`quick-ref.js` `spell('')`), but `.qr-speller-output` reserved `min-height: 40px` + `margin-top: 14px` ≈ 54 px of dead space below the input on every load (the empty band in the screenshot). Added `.qr-speller-output:empty { display: none; }` so the element collapses out of flow entirely when empty and expands back the instant phonetic items render — no layout jank, card height now fits its content.
- **feat(dashboard): rebuild the Today card as a triage surface (Next + Unfinished)** (May 15, 2026):
  The Today card re-listed reminders and expiring policies — pure duplication of the dedicated Reminders and CGL Compliance cards sitting right next to it on the bento grid. Reworked it ([js/dashboard-widgets.js](js/dashboard-widgets.js) `renderTodayWidget()`) into two non-redundant sections: **Next** — `_collectNextActions()` builds ONE ranked, deduped, ≤5 stream across domains in priority order (most-recent `ActivityLog` failure pinned → overdue reminders → policies expiring ≤7d, soonest, `isHidden`-aware → due-today reminders), each row clickable to its source; and **Unfinished** — `_collectUnfinished()` surfaces saved quote drafts (`QUOTES`/`INTAKE_V2_QUOTES`/`COMMERCIAL_QUOTES`) idle ≥ 2 days, oldest-first, cap 4, with a PL/V2/Comm source tag and "idle Nd" — the only place stalled in-progress work shows (the Clients card only lists *completed* quotes). The failure-pin / `×N`-coalesce behavior carries over into Next. Dead constants and the old `_collectTodayReminders`/`_collectTodayExpiring`/`_collectTodayActivity` collectors removed; new `.today-row` (clickable) + `.today-src-tag` CSS ([css/dashboard.css](css/dashboard.css)). `tests/today-view.test.js` render suite fully rewritten for the new design (14 tests: ranking, 5-cap, 7-day/`isHidden` window, staleness threshold + oldest-first + name derivation across all 3 stores, deep-link wiring, HTML escaping, feature-detection). Full suite green.
  Follow-up to the `cgl-utils.js` extraction. Three render-shaped methods (`renderLICCBBadges`, `renderClassificationOverride`, `renderNoteLog`) were *almost* pure — each had a single `this` dependency (`getClientCompliance` / `getNoteData`). Threading that value in as an argument makes them fully pure, so they moved into a new standalone `window.CglRenderers` IIFE ([js/cgl-renderers.js](js/cgl-renderers.js), ~93 lines), loaded after `cgl-utils.js` and before `compliance-dashboard.js`. `onclick="ComplianceDashboard.*"` handlers stay as string literals (resolved globally at click time — no behavior change). Call sites now pass the data explicitly: `CglRenderers.liCcbBadges(policy, this.getClientCompliance(policy.clientNumber))`, `CglRenderers.classificationOverride(policy, …)`, `CglRenderers.noteLog(pn, this.getNoteData(pn))` (5 note-log sites). **`compliance-dashboard.js`: 3,255 → 3,185** (cumulative 3,435 → 3,185, −250 / −7% across the two cuts). No public-API or behavioral change; no test referenced these. The `compliance-state-preservation` harness now `eval`s `cgl-renderers.js` alongside `cgl-utils.js`. AGENTS.md JS module count 107 → 108. Coverage: 6 compliance suites (354 tests) + full suite **63/64, 2432 tests** green (lone failure is the pre-existing unrelated `intake-v2` bindability test). The stateful render core (`renderPolicies`, the type/notify toggles, `_refreshNoteUI`) deliberately stays — it reads/writes module state and DOM and is not a safe pure-extraction target.
- **refactor(compliance): decompose the compliance-dashboard.js monolith — extract pure helpers to `js/cgl-utils.js`** (May 15, 2026):
  ...
  - **Extracted** ([js/cgl-utils.js](js/cgl-utils.js), new, ~208 lines): `escJsAttr`, `_hasAnnotationData`, `_smartMergeDict`, `_migrateNote`, `formatNoteTime`, `_noteIcon`, `_noteLabel`, `_noteIconHtml`, `_isLICCBApplicableType`, `_summarizeLILookup`, `_summarizeCCBLookup`, `_scrubUndefined`, `getStatusLabel`, `clientLink`, `_typeLabel`. Names preserved so the move was a mechanical `this.x()` → `CglUtil.x()` receiver swap (zero behavior change). `escJsAttr` is aliased (`const escJsAttr = CglUtil.escJsAttr;`) in the main file so the many `onclick="X('${escJsAttr(v)}')"` template sites are untouched.
  - **`compliance-dashboard.js`: 3,435 → 3,255 lines** (−180). The 14 object-method definitions removed; ~30 call sites rewired to `CglUtil.*`. The `_scrubUndefined` feature-detect guard (`this._scrubUndefined ? … : raw`) became an unconditional `CglUtil._scrubUndefined(raw)` (it is always loaded), and two stale Firestore/CloudSync comments on that path were corrected. `_safeLSWrite` and the stateful render/state methods deliberately stay (they touch module state — not safe to extract in this pass).
  - **No public-API or behavior change.** No production module referenced these via `ComplianceDashboard.*`. The 3 `compliance-state-preservation` tests that called `CD._hasAnnotationData` now test it at its new home (`window.CglUtil._hasAnnotationData`); the `audit-cleanup-fixes` source assertion now checks the definition in `cgl-utils.js` and the alias wiring in `compliance-dashboard.js`. Test harnesses that `eval` the dashboard now `eval` `cgl-utils.js` first (same rule as `js/utils.js`).
  - **Coverage:** 6 compliance suites (354 tests) green; full suite **63/64 suites, 2424 tests** (the lone failure is the pre-existing, unrelated `intake-v2` Progressive-bindability test, untouched by this change). A documented decomposition path remains for the stateful render layer (`renderLICCBBadges`/`renderClassificationOverride` become pure once `complianceRecord`/`applicable` are threaded as args).

### Removed
- **chore(bug-report): remove the in-app bug reporter end-to-end** (May 18, 2026):
  Small office — agents report issues directly, so the feature was pure overhead (a module + stylesheet + a `GITHUB_ISSUES_TOKEN` PAT dependency + a public-GitHub-Issues egress path to keep documented). Deleted `js/bug-report.js`, `css/bug-report.css`, and `tests/bug-report-scrub.test.js`; excised the `?type=bugreport` arm from [api/config.js](api/config.js) (the bug-report-only `handleBugReport` + `scrubBugReportPII` + `CATEGORY_LABELS` + `GITHUB_OWNER/REPO` consts + header/env-doc lines; `sanitizeInput`/`verifyAuthToken`/`securityMiddleware` stay — shared by `keys`/`phonetics`); removed the sidebar `BugReport.open()` button ([js/dashboard-widgets.js](js/dashboard-widgets.js)), the `index.html` `<link>`+`<script>`, both `sw.js` precache entries (required — else the version-bumped SW would 404 on precache), and the orphaned `.bugreport-pill` rules in [css/landing.css](css/landing.css) (dead — no markup referenced them). `docs/SECURITY.md` §4 tombstoned (heading/number kept so §5–§10 cross-refs stay valid) + the GitHub vendor/audit/verification rows removed so the security doc stays truthful. Doc inventories (`README.md`/`CLAUDE.md`/`QUICKREF.md`/`AGENTS.md`) + counts updated; `npm run audit-docs` clean. Full suite **65/66** green — exactly −12 tests (the deleted scrub suite) and −1 suite, no other regression; lone failure remains the pre-existing, unrelated `intake-v2` test. The now-unused `GITHUB_ISSUES_TOKEN` Vercel env var can be deleted at leisure (no code references it).

### Fixed
- **fix(css): legacy `.dark-mode` selectors that never matched (16 rules, 4 files)** (May 19, 2026):
  The app sets dark mode via `body.dark-mode` (CLAUDE.md), so a bare `.dark-mode .x { … }` rule **never applies** — those components silently kept their light styling in the default dark theme. Converted the 16 stragglers `.dark-mode ` → `body.dark-mode ` (line-start anchored, `body.dark-mode`/`[data-theme]` untouched): [css/components-inputs.css](css/components-inputs.css) (6 — carrier-autocomplete, ch-row, segmented buttons, toggles), [css/components-cards.css](css/components-cards.css) (6 — hero-export/buttons), [css/hawksoft.css](css/hawksoft.css) (2 — client picker), [css/ezlynx.css](css/ezlynx.css) (2 — send-hero button). `css/theme-professional.css` (8) deliberately **left** — its `.dark-mode` scoping is the theme's own internal selector, not a bug. Closes AGENTS.md §3.5/§5.9 known gaps. **Verified non-issue:** `paywall.css`/`phonetic-speller.css`/`components-quote-library.css` were flagged "zero dark rules" but are fully `var(--…)`-token-driven (zero hardcoded light surfaces) and adapt automatically via `variables.css` `body.dark-mode` token overrides — adding per-file dark blocks would be redundant churn, so they were intentionally not touched.
- **fix(intake-v2): the long-ignored "lone failing" bindability test was a stale expectation; harden `_resetDraft` silent save** (May 19, 2026):
  `tests/intake-v2.test.js` "auto with full applicant + auto + operator → bindable" failed every run and was waved off in CHANGELOGs as "lone unrelated failure" — eroding the meaning of "all green". Root cause (verified, not flaky): the test synced an operator from the applicant and asserted `progressive.ok`, but `OP_SYNC_PATHS` ([js/intake-v2-core.js](js/intake-v2-core.js)) deliberately does **not** copy `dl.*`, and `op-dl-num`/`op-dl-state` are `bindable: requiredBy('progressive',…)` ([js/intake-v2-fields.js](js/intake-v2-fields.js)) — you can't bind auto without a driver licence, so the **product behaviour is correct**; the test just never supplied the now-required DL fields (it only reasoned about VIN). Fixed the test to set the minimal realistic bindable set (`op.dl.num`/`op.dl.state`) — no product-logic change, no skip. Separately hardened the genuine data-loss silent-catch the reliability audit flagged: [js/intake-v2-core.js](js/intake-v2-core.js) `_resetDraft()` swallowed the post-reset `save()` error with `catch(_){}` then **unconditionally** logged ActivityLog `ok:true` (a false success — a swallowed failure means a reload could resurrect the old draft); now captures the outcome, `console.warn`s, and logs a `type:'error'` ActivityLog entry on failure (no control-flow change, reset UX unaffected). **Note:** the third part of this effort — a GitHub Actions test gate (`.github/workflows/test.yml`) so the suite actually blocks regressions — was built and verified locally but **not shipped**: this private repo has no Actions billing/minutes (every run failed in <13 s at the runner before `npm ci`, while local is 71/71 suites / 2622 tests green). The "no automated test gating — only the Vercel preview gates deploys" reliability gap therefore **remains open**; revisit if Actions is ever enabled, or via a local pre-push hook.
- **fix(compliance): CGL Dashboard shows data on open — no more mandatory Refresh** (May 19, 2026):
  Opening the Compliance page reliably showed nothing until the user clicked **Refresh** — every time. Two compounding root causes, both verified by direct read. **(1) Render-once-before-data-ready race.** `Auth._restoreSupabaseCloudAfterSignIn()` is fire-and-forget ([js/auth.js](js/auth.js):174 — `.catch()`, not `await`); `SupabaseSync.restoreFromCloud()` writes the cloud copy of `CGL_STATE` to localStorage ~1-3 s later then emits an `ActivityLog` `sync` event whose own comment ([js/supabase-sync.js](js/supabase-sync.js):456-468) says it exists to "wake the dashboard's ActivityLog subscriber so the … CGL cards re-render". `js/dashboard-widgets.js` (the bento card) subscribes — but `js/compliance-dashboard.js` (the actual plugin page) **never did**: `init()` read localStorage once, before the restore landed, and nothing re-read it. **(2) Over-strict cold-open cache gate.** `_loadFromAnyCache()` only returned a hit when `policies.length>0 && allPolicies.length>0`; a `policies`-only blob was logged "treating as stale" and returned `null`, so init fell through to the slow server-Redis-cached `/api/compliance` (empty on a cold device) instead of rendering instantly. **Fix:** added `_wireSyncReload()` + `_applySyncedReload()` to [js/compliance-dashboard.js](js/compliance-dashboard.js) (mirrors the proven `dashboard-widgets.js` pattern, wired once in `init()`) — subscribes to `ActivityLog`, reacts **only** to `{type:'sync'}` (the dashboard's own `{type:'save',area:'cgl'}` is ignored so it can't loop; `evt` is null-guarded for `clear()`), debounced via `Utils.debounce` to coalesce the restore burst, plus a cross-tab `storage` fallback. On fire it re-reads via the existing `loadState()` and repaints (cheap, no API) when rows exist, or pulls once if the page came up empty (cold-open self-heal). `_loadFromAnyCache()` now returns a `policies`-only blob flagged `_partialCache` instead of `null` so rows show immediately; `_showCachedData()` background-refreshes a partial cache so completeness self-corrects. Net effect: open the page → instant local render → synced annotations + full set fill in within seconds, **no manual Refresh**. New [tests/cgl-sync-reload.test.js](tests/cgl-sync-reload.test.js) (6 tests: sync repaints + picks up post-init `CGL_STATE` with no API call, save-event no-loop, cold-open self-heal, idempotent wiring, partial-cache return). Kept under the frozen `compliance-dashboard.js` size ceiling by reusing `loadState()` (no duplicated tier logic) rather than raising it; suite 70→71; `lint-aad` + `audit-docs` clean.
- **fix(api): declare `@supabase/supabase-js` — User Management 500 + silently-broken reminders cron** (May 18, 2026):
  User Management (new in #156) returned `Failed to load users (500)`. Root cause: `api/admin-supabase.js` and `api/_supabase-admin.js` `import { createClient } from '@supabase/supabase-js'`, but the package was **not declared in `package.json`** (deps were only `ioredis`/`pdf-lib`/`stripe`). Vercel builds each serverless function's `node_modules` from declared deps, so the import threw at invocation → the function crashed → a non-JSON 500 (which is why `admin-panel.js` showed the generic message instead of the endpoint's own JSON error). The client uses the SDK via CDN (`@supabase/supabase-js@2.105.4`), so the browser side worked and the gap stayed latent until a UI finally called the endpoint. **Also fixes the `reminders-sweep` daily cron**, which imports the same SDK through `api/_supabase-admin.js` and had therefore been failing (no daily digest) every day. Fix: add `"@supabase/supabase-js": "^2.105.4"` to `package.json` `dependencies` (version-matched to the client CDN). Audited every other bare server import — `canvg`/`dompurify`/`html2canvas` only appear inside the vendored `lib/jspdf.umd.min.js` browser bundle as jsPDF's own guarded optional `try{require()}catch` shims (never bundled as a Vercel function), so no other undeclared-dep crashes exist. Full suite **66/67, 2491** green (lone failure pre-existing unrelated `intake-v2`); `audit-docs` clean.
- **ui(account): Sync Now button now shows visible feedback** (May 18, 2026):
  Bare `CloudSync.fullSync()` ran with zero UI feedback (and is a deliberate no-op for non-admins by agency policy / when MFA is unmet), so the button looked dead even when it worked. New `CloudSync.syncNowUI(btn)` ([js/sync-facade.js](js/sync-facade.js)) disables the button + shows "Syncing…" while in flight, then toasts the outcome — admin-policy / MFA-required / error / success — and stamps `#authSyncTimestamp` so something visibly changes. Button onclick rewired in [index.html](index.html). The keep-data-local toggle already toasts via `setDisabled`.
- **fix(quickref): Quick Emoji labels no longer clip descenders** (May 18, 2026):
  The prior emoji-card shrink (PR #155, `height:64px`) was too aggressive: `.qr-emoji-label` has no `line-height` (UA ~1.2 box) and 64px left only ~41px content (border 3 + padding 20) after the 26px icon + 4px gap, so descenders (the "g" in Logged/Pending/Waiting, "y" in Priority) were clipped. Fix ([css/quickref.css](css/quickref.css)): add `line-height:1.25` to `.qr-emoji-label` (predictable box that contains the descender) and bump `.qr-emoji-btn` `height`/`min-height` 64→70px (still 22px below the original 92px — stays compact). `overflow:hidden`/ellipsis kept for the horizontal clamp. CSS-only.
- **fix(dashboard): Reminders + CGL cards now update live (were stale until you opened the source page)** (May 18, 2026):
  The dashboard Reminders and CGL cards only reflected reality after you visited the Reminders / Compliance *page* — a side effect of the lazy-loaded plugin architecture, not an intentional design. `renderRemindersWidget()` ([js/dashboard-widgets.js](js/dashboard-widgets.js)) read the `Reminders` module's *in-memory* state, which is only hydrated by `Reminders.init()` — and `init()` only runs when the Reminders plugin page is opened. Before that (or after a cloud restore blind-overwrote the stored blob) the card showed empty/stale counts; the 60 s auto-refresh in `showDashboard()` re-rendered but from the *same stale memory*, so nothing visibly changed. CGL was adjacent: `renderComplianceWidget()` reads localStorage directly so synced `CGL_STATE` changes did surface within 60 s, but no path re-rendered it on a cloud pull, and the vault-unlock restore path doesn't call `refreshAll()`.
  - **New `Reminders.refreshFromStorage()`** ([js/reminders.js](js/reminders.js)): a pure re-hydrate (`_load({ quiet: true })`) with **no persist/push/log side effect**. `_load`/`_reconcileFromLedger` now take an opts arg; `quiet` heals the in-memory ledger state (so widget counts are correct) but skips the reconcile's `_save()` — which would otherwise emit an ActivityLog event + schedule a Supabase push, and on the new ActivityLog-subscribed render path would re-enter ActivityLog and churn sync. `renderRemindersWidget()` calls it on every render (init / 60 s / return-home / activity), so the card always mirrors current stored/synced state.
  - **New one-time ActivityLog subscription** ([js/dashboard-widgets.js](js/dashboard-widgets.js)) re-renders the Reminders + CGL cards (and badges) on any save/sync/import while the dashboard is visible — the same guarded, loop-safe pattern the Today widget already uses (all three render fns emit no ActivityLog events; verified `updateBadges` too).
  - **`restoreFromCloud()`** ([js/supabase-sync.js](js/supabase-sync.js)) now emits a `sync` ActivityLog event when it actually writes docs — making cloud *pulls* visible in the header pill / activity panel like pushes already were, and instantly waking the dashboard subscriber so a restore (incl. the vault-biometric-unlock path that never called `refreshAll()`) refreshes the cards immediately.
  - The production-only hourly HawkSoft policy fetch and its localhost gate are unchanged (HawkSoft API is unreachable on localhost — out of scope per the chosen approach). One source-grep invariant in [tests/reminders-enhancements.test.js](tests/reminders-enhancements.test.js) relaxed to be arg-agnostic (`_reconcileFromLedger(` — it now forwards opts). Full suite **66/67 suites, 2491 tests** green (lone failure is the pre-existing, unrelated `intake-v2` Progressive-bindability test, untouched here); `pretest` AAD lint: 0 violations; `npm run audit-docs` clean.
- **fix(api): upstream timeout on AI proxy fetches — no more hung serverless functions** (May 16, 2026):
  A "vibe-coded app review" checklist audit (committed secrets / RLS / rate limiting / error handling) found Altech passes 3 of 4 cleanly — but the reviewer's exact #4 scenario, "a third-party API times out and nothing catches it", was real: every upstream AI `fetch()` (Anthropic / Gemini / OpenAI / OpenRouter) had **no `AbortController`/deadline**. A slow provider held the Vercel function open for the full 300 s `maxDuration`, burning compute and giving the user a multi-minute spinner ending in an opaque 500. The pattern already existed in the codebase (`api/_apify-client.js`, `api/_property-flood.js` hand-roll a timeout) — it just wasn't applied to the AI calls. New shared helper [api/_fetch-timeout.js](api/_fetch-timeout.js) (`fetchWithTimeout`, default 30 s, well under `maxDuration` so the function returns a clean error instead of being killed; plus `isUpstreamTimeout` / `sendUpstreamTimeout`). Applied to all **11** AI fetch sites: [api/_ai-router.js](api/_ai-router.js) (8), [api/anthropic-proxy.js](api/anthropic-proxy.js) (1), [api/vision-processor.js](api/vision-processor.js) (2). On timeout the routable handlers (`anthropic-proxy`, `vision-processor`, `property-intelligence`, `historical-analyzer`, `prospect-lookup`) now return **504 + `Retry-After: 30`** — which the client's existing `js/ai-provider.js` `_decodeApiError()` already maps (case 408/504) to a friendly "timed out — try again in a moment" toast (verified; no client change needed). Helpers with no fallback tier (`historical-analyzer` analyze\*, `vision-processor` documentIntel) **re-throw** the tagged timeout instead of swallowing it into a soft "failed" result, since a timeout is explicitly retryable. The property-intelligence tier-fallback helpers (Rentcast → Apify → Gemini) deliberately keep their degrade-to-next-source behavior — there a slow Gemini *should* fall through, not 504 the whole request. New [tests/fetch-timeout.test.js](tests/fetch-timeout.test.js) (12 tests): `fetchWithTimeout` happy-path passthrough / timeout→tagged-error / non-timeout-error propagation, `isUpstreamTimeout` / `sendUpstreamTimeout` shape, and anthropic-proxy handler timeout→504+Retry-After / 200 passthrough / non-timeout→500-preserved. Full suite **66/67 suites, 2491 tests** green (lone failure is the pre-existing, unrelated `intake-v2` Progressive-bindability test, untouched here). `pretest` AAD lint: 0 violations.
- **fix(compliance): batch L&I/CCB verification into one save (was flooding the Activity log)** (May 15, 2026):
  The Activity log filled with "CGL state saved (Nv · Nd · Nn)" entries that kept climbing. Root cause: `verifyClient()` ([js/compliance-dashboard.js](js/compliance-dashboard.js)) called `this.saveState()` **per client**, and `verifyAllClients()` re-drains the L&I/CCB queue on **every dashboard open** (`_showCachedData` / `fetchPoliciesFromAPI` / `showLocalDevMode`) — so ~30+ applicable clients meant ~30+ `saveState()` calls per load, each one an IndexedDB + localStorage + KV write **and** a debounced Supabase push (the interleaved "Synced 12 docs to Supabase" / "Reminders saved" rows). Coalescing (the earlier ActivityLog fix) collapsed *consecutive* dups but the interleaving broke the runs, so the count just climbed. Fix: `verifyClient()` no longer saves per client — it updates `clientCompliance[cn]` in memory and sets a `_verifyDirty` flag; `_drainVerifyQueue()` flushes **exactly one** `saveState()` when the queue fully drains (nothing queued, nothing in flight). Per-client `filterPolicies()/updateStats()` re-renders stay (badge responsiveness; they don't persist anything). Net: N writes/pushes per load → **1**; once a client's `verifiedAt` is persisted, the 24h skip means subsequent opens queue nobody and save nothing. Regression test in [tests/compliance-state-preservation.test.js](tests/compliance-state-preservation.test.js): 3 verifiable clients drained → asserts `clientCompliance` has all 3 but `saveState` fired exactly once. 4 compliance suites (157 tests) + full suite **65/66, 2483** green (lone failure pre-existing unrelated `intake-v2`).
- **fix(compliance): dashboard renders the local cache instantly on open (no more manual Refresh)** (May 15, 2026):
  Opening the CGL/Commercial dashboard sat on the "Loading compliance data… Connecting to HawkSoft" spinner until the user clicked the top **Refresh** button, every time — even though a valid local cache existed (stats/counter/legend were already populated from it). Root cause: `_loadFromAnyCache()` ([js/compliance-dashboard.js](js/compliance-dashboard.js)) gathered all four cache sources with `Promise.allSettled([disk, idb, localStorage, kv])` and only returned **after the slowest settled** — the `kv` source is a network round-trip to `/api/kv-store` (cold-start / no client-side timeout), so every open blocked on it despite localStorage/IDB being ready in milliseconds. The top Refresh "worked" only because `forceRefresh()` bypasses the cache path and hits the API directly. Fix: await the sources **fastest-first** and return on the first valid hit — localStorage (instant) → IDB (≤3 s, own timeout) → only then `Promise.allSettled([disk, kv])` as a fallback when no local cache exists. All four promises are still kicked off in parallel (the slow ones keep warming in the background), and `_showCachedData()`'s existing staleness check still schedules a silent background API refresh, so data stays current without a manual Refresh. Regression test in [tests/compliance-state-preservation.test.js](tests/compliance-state-preservation.test.js): seeds a valid localStorage cache, stubs `_loadFromKV` to **never resolve**, asserts `_loadFromAnyCache()` still resolves from localStorage (the pre-fix `allSettled` would hang the test). 4 compliance suites (245 tests) + full suite **65/66, 2482** green (lone failure pre-existing unrelated `intake-v2`).
- **fix(compliance): legend no longer bleeds out of its card; stat tiles fit one row** (May 15, 2026):
  Two CGL/Commercial dashboard layout bugs. (1) **Legend overflow** — `.cgl-legend-item` was `flex: 1 1 0` + `white-space: nowrap`, which forced all six legend chips onto a single stretched row; the long "🛠️ Needs L&I/CCB — Contractor — update lni.wa.gov / ccb.state.or.us" item couldn't shrink (nowrap) and spilled past the card's right edge. Now `flex: 0 1 auto` + `white-space: normal` so chips are content-sized and the grid's existing `flex-wrap` wraps them *inside* the card at any width ([css/compliance-main.css](css/compliance-main.css)). (2) **Stat row** — `.cgl-stats-grid` was `repeat(6, 1fr)` but there are **7** tiles (Total / Critical / Expiring / Expired / Manual / Updated / Needs L&I/CCB), so the 7th wrapped to its own row leaving a near-empty band. Bumped to `repeat(7, 1fr)` in the base + desktop rules ([css/compliance-main.css](css/compliance-main.css), [css/compliance-responsive.css](css/compliance-responsive.css)); mobile 3-/2-col stacking unchanged. CSS-only; full suite **65/66, 2481** green (lone failure pre-existing unrelated `intake-v2`). Not visually verified in-env — eyeball the PR preview / hard-reload.
- **fix(compliance): policies with no policy number can finally be dismissed (kept resurfacing)** (May 15, 2026):
  A HawkSoft policy that arrives with **no policy number** (binders, data gaps — e.g. an expired CGL like "2 R Construction LLC") could never be dismissed, snoozed, verified, or noted: every annotation is keyed by `policyNumber`, `renderPolicies` built `rawPolicyNumber = String(policy.policyNumber || '')` → `''`, the action onclicks fired `dismissPolicy('')`, and the empty-key guards (`if (!policyNumber…) return;`, added to stop a Firestore-era `''`-key crash) turned every such action into a **silent no-op**. The policy stayed un-hideable and, being expired, kept sorting back to the top of the list. New `_normalizePolicyKeys()` ([js/compliance-dashboard.js](js/compliance-dashboard.js)) runs after every policies load (`_showCachedData` / `fetchPoliciesFromAPI` / `showLocalDevMode`) and gives any number-less policy a **stable synthetic key** — `NOPOL-<hawksoft policyId>`, or a deterministic `NOPOL-<clientNumber>~<carrier>~<exp>` when even the id is absent — so it keys, hides, and **persists/syncs** exactly like a normal policy. Idempotent; real policy numbers are untouched (zero migration — these rows were never dismissable, so there's no stored state to convert). `renderPolicies` still shows the Policy # cell **blank** for synthetic keys (`_syntheticPolicyNumber` flag) so the UI is unchanged. The empty-key guards stay as defensive backstops. Coverage: 2 regression tests in [tests/compliance-state-preservation.test.js](tests/compliance-state-preservation.test.js) (synthetic-key assignment + idempotency + dismiss→hidden→persists; id-less fallback + real-number passthrough); 4 compliance suites (155 tests) + full suite **65/66, 2481** green (lone failure is the pre-existing unrelated `intake-v2` test).
- **fix(sw): precache `cgl-utils.js` + `cgl-renderers.js` — CGL dashboard stuck on the static loader** (May 15, 2026):
  After the compliance-dashboard.js decomposition (extracting `js/cgl-utils.js` and `js/cgl-renderers.js`), the CGL dashboard could hang forever on the static "🛡️ Loading compliance data… / Connecting to HawkSoft" screen until a manual reload. Root cause: `sw.js` `APP_SHELL` precaches `compliance-dashboard.js` (and its split-precedent helpers `compliance-idb.js` / `hawksoft-renderers.js`) but the decomposition **never added the two new modules to that list**. `compliance-dashboard.js`'s IIFE resolves `const CglUtil = window.CglUtil; const escJsAttr = CglUtil.escJsAttr;` (and `window.CglRenderers`) at **eval time** — a new hard load-order dependency that didn't exist before the refactor. Whenever the service worker served the precached shell (offline, a network blip on those two files, or the post-deploy activation window), a *precached* `compliance-dashboard.js` ran against *absent* dependencies → `TypeError` → `window.ComplianceDashboard` never created → `navigateTo('compliance')` never initializes → static loader forever. A reload that re-fetched the missing modules from network made it work, which read as "it no longer local-caches." Fix: add `/js/cgl-utils.js` and `/js/cgl-renderers.js` to `APP_SHELL` next to `compliance-idb.js`, restoring the invariant that a precached dependent always has its precached dependencies (the `CACHE_VERSION` pre-commit bump forces clients to re-precache the corrected shell). Network-first keeps online users unaffected by the ~50 *other* pre-existing APP_SHELL omissions — only this dependent/dependency asymmetry was fatal.
- **fix(home): "Recent" / Activity log no longer drowns in duplicate "CGL state saved" rows** (May 15, 2026):
  The Today card's Recent column (and the slide-out Activity panel) showed nothing but a wall of identical `CGL state saved (Nv · Nd · Nn)` entries. Root cause: `ComplianceDashboard.saveState()` fires from ~20 call sites (every policy verify/dismiss/note, plus bulk operations), and each one pushed a fresh row into the 100-slot `ActivityLog` ring buffer — evicting every meaningful event (syncs, exports, AI, errors) within seconds.
  - **Coalescing at the source** ([js/activity-log.js](js/activity-log.js)). `ActivityLog.add()` now collapses consecutive identical events (same `type`+`area`+`message`+`ok`) within a 5-minute window into the existing head entry: `count` increments, `ts` moves forward, `detail` refreshes, subscribers still fire each time. Mirrors the existing `App.toast` dedupe. Fresh entries carry `count: 1`; a burst of 30 saves is now one row reading `×30`. The slide-out panel renders the `×N` badge.
  - **Smarter Recent** ([js/dashboard-widgets.js](js/dashboard-widgets.js)). `_collectTodayActivity()` no longer just `slice(0, 5)`s the log: (1) the most recent failure is **pinned on top** so an error never gets buried under a wall of saves; (2) routine successful `save` rows are **capped at one** so exports/syncs/AI/errors keep their slots; (3) leftover slots backfill newest-first so the list is never artificially short when saves are genuinely all there is. A `×N` count badge renders next to coalesced rows ([css/dashboard.css](css/dashboard.css), `.today-activity-count` — `--text-secondary`/`--bg-input` tokens, tabular-nums, sits outside the ellipsized title so it never clips).
  - **Coverage:** 5 new `ActivityLog` tests (count:1 default, 30→×30 coalescing, no-coalesce on differing type/area/ok, ts+detail refresh + subscriber fan-out, post-window fresh row) + 3 new Today-widget tests (buried failure pinned + flagged, routine-save cap keeps a buried sync visible, ×N badge renders). Full suite: **63 suites / 2432 tests green** (1 pre-existing unrelated `intake-v2` Progressive-bindability failure, untouched by this change).
- **fix(reminders): completed 'once' task no longer resurfaces as "Past due" the next day** (May 15, 2026):
  A `frequency: 'once'` reminder that the user had completed kept coming back as "Past due" after a day. Root cause was a cross-session data-loss path, not a status-logic bug: `_getStatus()` correctly returns `'completed'` forever for a `once` task that has any completion — but the completion was being **wiped between sessions**.
  - **Root cause** ([js/supabase-sync.js](js/supabase-sync.js) `restoreFromCloud()` → [js/auth.js](js/auth.js) `_restoreSupabaseCloudAfterSignIn`). `restoreFromCloud()` blind-overwrites the `reminders` localStorage blob with the cloud copy *before* `Reminders.init()` runs. `App.save()` schedules the cloud push on a 3 s debounce, so a completion made just before a tab close (or while offline, or superseded by a stale push from another device) never reaches the cloud. The next sign-in then restores the pre-completion blob over it. Recurring tasks mask this — `_autoAdvanceRecurring()` rolls their `dueDate` forward so a lost completion just looks like a fresh cycle — but a `once` task has no such healing, so the lost completion shows "Past due" permanently. The sync module is opaque-blob by design ("never inspects payload structure"), so the fix belongs in the reminders module.
  - **Fix — local-only completion ledger** ([js/reminders.js](js/reminders.js), [js/storage-keys.js](js/storage-keys.js)). New `STORAGE_KEYS.REMINDERS_COMPLETIONS` mirrors each task's completion list per device. It is deliberately **NOT** in `DOC_LOCAL_KEYS`, so `restoreFromCloud()` never iterates it and the cloud copy can't clobber it (same local-only rationale as `ACTIVITY_LOG`). `completeTask` / `uncompleteTask` / `undoComplete` write the ledger; `deleteTask` prunes it (so a deleted task can't be resurrected). `_load()` calls a new `_reconcileFromLedger()` that, for any task the ledger has an entry for, **replaces** the just-loaded blob's `completions` with the ledger's list. Replace (not union) keeps both a completion AND a later uncomplete durable against a stale overwrite; a completion made on another device still arrives via the synced blob for tasks this device has never touched (no ledger entry → blob wins). When a heal happens the state is re-saved so the recovered completion re-propagates on the next push.
  - **Defensive:** `completeTask` / `uncompleteTask` now guard `task.completions` with `Array.isArray` before `push`/`pop` — a blob from an older client that lacks the array would otherwise throw inside `completeTask`, silently dropping the completion (same observable symptom).
  - **Coverage:** 3 behavioral tests (complete → stale-blob restore → still completed; never-completed → still Past due; uncomplete → honored over a stale-completed cloud copy) + 3 source-level wiring invariants in [tests/reminders-enhancements.test.js](tests/reminders-enhancements.test.js). Full suite: **63 suites / 2424 tests green** (1 pre-existing unrelated `intake-v2` bindability failure, untouched by this change).

- **fix(compliance): audit-driven cleanup pass** (May 15, 2026):
  Focused pass on the CGL/compliance dashboard after re-walking the live screen. Seven user-visible bugs and a handful of defensive guards.
  - **`Needs State Update` filter was overmatching.** ([js/compliance-dashboard.js](js/compliance-dashboard.js)) The pre-fix predicate was `!getNoteData(pn)?.stateUpdated` — true for nearly every policy because most have no `stateUpdated` note ever. Now routes through the existing `_needsStateUpdate(policyNumber)` helper, which only fires for renewal rows that actually need action.
  - **Snoozed policies didn't behave like hidden rows.** ([js/compliance-dashboard.js](js/compliance-dashboard.js)) The row's `isHidden` flag only counted verified + dismissed, so a snoozed row could leak through normal filters and the action column rendered the wrong button set. Now `isHidden = isVerified || isDismissed || isSnoozed`, and the snoozed branch renders the `💤 Until <date>` badge + `Wake` button instead of the dismiss/verify pair.
  - **Search could crash on partial policy records.** ([js/compliance-dashboard.js](js/compliance-dashboard.js)) `policy.clientName.toLowerCase()` threw on policies where `clientName` was null — uncommon but real, and the whole table went blank when it happened. Wrapped each of the three search fields in `String(... || '')` so a missing value falls through silently.
  - **Visible counts ignored hidden policy-type toggles.** ([js/compliance-dashboard.js](js/compliance-dashboard.js)) The "Showing X of Y" counter's `visibleTotal` only excluded verified/dismissed/snoozed rows, not policy types the user had toggled off in the chip row. Now also filters out `this.hiddenTypes`.
  - **Policy-number values were reused unsafely across JS handlers and HTML ids/attrs.** ([js/compliance-dashboard.js](js/compliance-dashboard.js)) The same `pn = escJsAttr(...)` binding was being interpolated into both `onclick="X('${pn}')"` (where backslash-escape is required) AND `id="state-badge-${pn}"` (where backslash-escape produces an invalid id). Split into three named bindings: `rawPolicyNumber` for data lookups, `pn` (`escJsAttr`) for JS-string interpolation, `pnAttr` (`Utils.escapeAttr`) for HTML attribute values. `renderLICCBBadges` + `renderClassificationOverride` already had a hand-rolled `cn` escape — both now route through the shared `escJsAttr` helper.
  - **"Crime" policies were being misclassified as Inland Marine.** ([lib/compliance-utils.js](lib/compliance-utils.js)) `getCommercialPolicyType` tested `/inland|im/i` before `/crime/i`, and "im" is a substring of "crime", so any Crime policy short-circuited to `'im'`. Same family on `/wc/` (matched inside many strings) and `/cp/` (matched inside "carpet", "scope", etc.). Added word boundaries: `/inland(?:\s+marine)?|\bim\b/`, `/workers|\bwc\b|work comp/`, `/prpty|commercial property|comm prop|\bcp\b|bldg|building/`. New `getCommercialPolicyType` test asserts Crime stays Crime.
  - **Filter labels didn't match actual date ranges.** ([plugins/compliance.html](plugins/compliance.html)) Dropdown read "Critical (<14d)" and "Expiring Soon (14-60d)" but `getExpirationStatus()` actually uses 0–5 / 6–59. Labels now read "Critical (0-5d)" / "Expiring Soon (6-59d)" — matches the code.
  - **Bonus defensive fixes:** `saveNote` and `scheduleAfterNoteSave` use `getElementById` / `dataset.noteFor` instead of `querySelector` with policy-number-as-selector — robust against `#`, `.`, and other CSS-special chars in policy numbers. `deleteNoteEntry` call site now uses `escJsAttr` for the policy-number argument (was using plain `Utils.escapeHTML`, which produced JS-string-broken output if a policy number contained an apostrophe).
  - **Coverage:** new `getCommercialPolicyType > "crime" not inland marine` test in `tests/api-compliance.test.js`; two new assertions in `tests/audit-cleanup-fixes.test.js` for the snoozed-isHidden + needs-state-update filter changes. **Total: 4 suites, 153 tests pass.**

- **fix(intakev2): synced-op disclosure auto-opens + right-rail still cramped at 1536 px** (May 15, 2026):
  Two follow-ups to the audit-driven sweep above, surfaced by an immediate re-test on the merged build.
  - **Synced operator disclosure was opening too early** ([js/intake-v2-operators.js](js/intake-v2-operators.js)). `syncApplicantOperators()` seeds the primary operator's `relationship` to `'Self'` at card creation, which the new `_hasAnyAutoSpecificValue` heuristic was reading as user input and popping the License/MVR/history `<details>` open before any auto / boat / RV existed — defeating the point of the compact view. Fix: drop `relationship` from the "auto-specific" field set for both primary AND co-applicant (was previously excluded only for co-applicant). For the primary it's always `'Self'`; for the co-applicant it's mirrored from the Quick Start co-applicant block. Neither belongs in the auto-specific bucket since neither tells the agent anything about whether to expand the panel. Side benefit: `relationship` also no longer renders inside the disclosure for either synced card.
  - **Right rail still re-appeared at 1536 px** ([css/intake-v2.css](css/intake-v2.css)). The 1500 px threshold from the first sweep was too low — at the very common 1536 px effective width (1920×1080 at 125% scale) the talk track came back and DOB / state / ZIP started truncating again. Raised to 1700 px so the talk track also clears 1536 px and 1680 px laptop screens; 1920 px desktops still get the talk track. Left-rail breakpoint (1280 px) unchanged.

- **fix(intakev2): false carrier-ready, rail crowding, modal pile-up, synced-op duplication** (May 15, 2026):
  Audit-driven UX sweep covering the four highest-impact complaints from the post-PR-#136 v2 walkthrough.
  - **Carrier "ready to quote" before a product exists** ([js/intake-v2-bindability.js](js/intake-v2-bindability.js)). Pre-fix, an agent who filled out applicant + address but never added a home/auto/boat/RV saw every carrier flip to ✓ — a trust-breaking false-positive since there's literally nothing for the carrier to write. `computeBindability` now pushes a synthetic `__products__` missing entry per carrier when no product line is active AND sets `info.blocked: true`. `renderTopbarStatus` in [js/intake-v2-layout.js](js/intake-v2-layout.js) skips the "1 field left" amber state when `blocked` is set — the missing piece isn't a field the agent can grab on the same call, it's a structural decision. `_resolveMissingDomId` jumps the popover click to the Properties section (`#iv2-properties`). New regression test in [tests/intake-v2.test.js](tests/intake-v2.test.js) asserts every carrier reports `ok:false`, `blocked:true`, and a `__products__` missing entry when applicant+address are filled but no product line exists.
  - **Rails collapse threshold raised from 1100 → 1500/1280** ([css/intake-v2.css](css/intake-v2.css)). At 1280 px (a common 13"–15" laptop width), the 256 px app sidebar plus the v2 left rail (200 px) plus the talk-track rail (280 px) plus gutters ate roughly 800 px — DOB / phone / email / city fields visibly truncated. Two-tier collapse: at ≤ 1500 px the right rail (talk track) hides first (it's the widest and least essential; talk-track prompts still surface inline via [js/intake-v2-talktrack.js](js/intake-v2-talktrack.js) on field focus); at ≤ 1280 px the left rail also hides (J/K shortcuts + topbar section indicators duplicate the jump anchors). Both rails are `position: fixed` so `display:none` is enough — no grid rewrite needed until the single-column fallback.
  - **Keyboard-shortcuts modal no longer auto-opens on first visit** ([js/intake-v2-keyboard.js](js/intake-v2-keyboard.js)). Pre-fix, new users hit the onboarding wizard, the access-code prompt, AND the shortcut help modal back-to-back-to-back before they could see the intake form. The shortcut surface (N add, J/K nav, Alt+L defer, Alt+1..6 jump) is still discoverable — `?` keyboard shortcut, "?" icon in the topbar, and the command palette — it just no longer ambushes new users. Removed the `HELP_SHOWN_KEY` localStorage flag and the `_maybeAutoShowHelp()` boot hook entirely.
  - **Synced applicant / co-applicant operator card compacted** ([js/intake-v2-operators.js](js/intake-v2-operators.js) + [css/intake-v2.css](css/intake-v2.css)). Pre-fix, auto-creating the primary operator from the Quick Start applicant block was good — but the resulting operator card rendered all ~22 fields, with the ~9 synced ones (firstName/lastName/dob/gender/maritalStatus/occupation/education + relationship for co-applicant) shown as readonly duplicates. Net visual cost: a long heavy block right after the Quick Start that just repeats data above it. The synced card now hides the duplicates entirely and tucks the auto-specific fields (DL, MVR, history flags, discounts) into a collapsible `<details>` with summary "License, MVR & history — only needed when adding an auto, boat, or RV". Details auto-expands when `data.autos` / `data.boats` / `data.rvs` is non-empty OR any auto-specific field already has a value. Manual collapse/expand persists across re-renders via a module-level `_syncedExpandState` Map keyed on `op.id` (without it, every operator-link change would snap the panel back to the heuristic).
...
- **feat(intakev2-pdf): fact-finder layout + summary/fact-finder toggle** (May 14, 2026):
  ...
  - **Summary** (default, unchanged from PR #110): underwriting snapshot on page 1 (carrier-fit grid, severity-density risk callouts, coverage A/B/C/D ratio bar, action-items checklist) + dense detail dossier on pages 2+. For at-a-glance review.
  - **Fact Finder** (new): compressed page-1 cover (hero + 2×2 carrier-fit + top-3 risks + top-3 actions + "Fact Finder" instruction strip) + numbered sections §1–§N matching the **EZLynx Personal Lines screen order** ([js/ezlynx-tool.js](js/ezlynx-tool.js) `formFields[]`, lines 59-86). Section order: Applicant → Co-Applicant → Address → Household → Auto Policy → Auto Coverage → Residence → Drivers → Vehicles → Home Dwelling → Home Coverage → Home Endorsements → Mortgagee/Lien → Boats & RVs → History → Discounts → Producer Action Items → Agent Notes. Field labels use EZLynx terminology verbatim ("Bodily Injury", "Property Damage", "Personal Liability", "Feet from Hydrant", "Med Payments Auto") so the agent pattern-matches PDF text to the carrier UI without translation.
  - **Builder API**: `IntakeV2PDFBuilder.buildIntakeV2PDF(data, { layout: 'summary' | 'factfinder' })`. Both layouts share the header, hero card, footer, compute helpers, and visual primitives — only the body branches. Filenames suffix with `_summary` or `_factfinder` so agents can keep both side-by-side without overwriting.
  - **UI plumbing**: Review section adds an "Export PDF · Fact Finder" button next to "Export PDF Summary"; both are also accessible via Command Palette (`Cmd+K → Intake v2 — Export PDF Summary / Fact Finder`).
  - **Section numbering renumbers as drawn** — auto-only quotes show §1–§13, full-line bundles can reach §18. Empty sections are skipped entirely (no orphan headers). Cover instruction strip says "Numbered sections below mirror the EZLynx Personal Lines screen order".
  - **Polish during the 7-scenario QA loop**: tightened ratio-bar segment labels (single-letter B/C/D so even Cov-B-at-5% segments get a visible label inside the bar, with an abbreviated `$Xk` form when the dollar amount can't fit); shrunk label font in 3-col mode (6.5pt) so long EZLynx labels like "CREDIT CHECK AUTH" / "UM PROPERTY DAMAGE" don't collide with values; widened the 3-col labelW from 0.42 to 0.48 of the cell; removed the duplicate "RISK FLAGS" sectionLabel above the severityBox (the box's own header is the divider); reformatted Property Damage from "$100k" to "$100,000" (the dollar amount EZLynx actually expects); fixed boat market-value formatting in risk markers (`$58000` → `$58,000`).
  - **Tested across 7 scenarios** × 2 layouts (14 PDFs): auto-only minimal, auto-only busy (3 vehicles + 4 ops + SR-22 teen), home-only minimal, home-only multi-home, RV-only, boat-only (wood hull), full bundle (home + 2 autos + boat + RV). All layouts render cleanly with proper page breaks, conditional sections, EZLynx-aligned field ordering.
  - **Tests**: 26/26 intake-v2 + 49/49 v1 export contracts pass. The default `buildIntakeV2PDF(data)` call (no opts) still routes to `'summary'`, preserving the JSDOM sentinel test ("Yamaha", "YAM12345678X", "Jayco", "Class fifthWheel" still flow through summary-mode `assetCard` titles).

- **feat(intakev2): native CMSMTF + EZLynx XML builders, redesigned PDF** (May 14, 2026):
  v2 was previously routing CMSMTF and EZLynx XML exports through `intake-v2-export-map.js → App.buildCMSMTF / App.buildEZLynxXML / App.buildEZLynxHomeXML`. The legacy bridge silently dropped roughly 35 fields that v2 actually captures — most critically the home deductible inside `<DeductibeInfo/>` (emitted empty by v1's home XML builder), per-asset lien holders, SR-22 / FR-44 filings per operator, CreditCheckAuth, YearsWithContinuousCoverage, vehicle telematics/TNC/anti-theft, per-driver Industry/Occupation/Education, mortgagee address + loan number, multi-home data, and (in EZHOME) PolicyType, NumberOfOccupants, DwellingOccupancy, NumberOfFullBaths/HalfBaths, DistanceToFireStation, SwimmingPool, DogOnPremises, Trampoline, BusinessOnPremises, Foundation, RoofDesign, all `*UpdateYear` indicators, PurchaseDate, PreferredContactMethod, YearsAtAddress, FirstMortgagee.
  - **[js/intake-v2-export-cmsmtf.js](js/intake-v2-export-cmsmtf.js)** — native CMSMTF builder reading v2's nested tree directly. Emits SR-22 filings (`drv_bFiling` + `drv_sFilingState` + `drv_sFilingReason`), license-suspension/exclusion flags via `drv_bExcluded`/`drv_bNonDriver`, defensive-driving / mature-driver / good-student bonuses, per-vehicle loss-payee blocks (`veh_bLossPayee` + `veh_sLossPayeeName`/`Address`), mortgagee address + loan number (`gen_sLpAddress1` + `gen_sLpLoanNumber1`), DateLicensed derived from DOB + `yearsAuto`, full discount + affinity dump into misc data, prior-insurance details split into `gen_sFSCNotes`, multi-home rollup, and the typo-aware `gen_sDecuct` AND `gen_sDeduct` both emitted so whichever HawkSoft parses we're covered. LOB derivation picks HO3 vs HO6 (condo) vs HO4 (renter) from `dwellingType`.
  - **[js/intake-v2-export-ezlynx.js](js/intake-v2-export-ezlynx.js)** — native V200 EZAUTO + EZHOME XML builders.
    - **EZAUTO** adds CreditCheckAuth, YearsWithContinuousCoverage (Months too), per-driver Industry/Occupation/Education, SR22/FR44/LicenseSuspendedRevoked/DriverTraining/DistantStudent, vehicle Ownership/PurchaseDate/OriginalOwner/ExistingDamage/LicensePlate/PlateState, DaysPerWeek inside VehiclesUse, MedPay inside GeneralCoverage, history-derived `<Violation>` and `<Loss>` records keyed to operators, proper VehicleAssignment cross-references built from a driverIdxById map.
    - **EZHOME** is the big fix — the empty `<DeductibeInfo/>` is no longer empty (it now correctly wraps `<Deductible>{cov.deductible}</Deductible>`). Adds NumberOfOccupants, DwellingOccupancy, NumberOfFullBaths/HalfBaths, DistanceToFireStation, SwimmingPool, DogOnPremises, Trampoline, BusinessOnPremises, Foundation, RoofDesign, `*Update`/`*UpdateYear` for roof/electrical/plumbing, PurchaseDate, PolicyType (HO3/HO4/HO6 from dwelling), CreditCheckAuth, YearsAtAddress, PreferredContactMethod, Industry/Occupation/Education per-applicant, FirstMortgagee Yes/No flag, ReplacementCost coverages A-F + PersonalLiability + MedicalPayments, ICQ-style ExtendedInfo block with per-endorsement valuepairs (waterbackup/serviceline/equipbreakdown/ordinanceorlaw/identitytheft) so EZLynx's importer surfaces them as carrier preferences. Construction string normalization + roof-type enum normalization preserved so V200's deserializer doesn't 500 on Altech's verbose enum values.
  - **[js/intake-v2-export-pdf.js](js/intake-v2-export-pdf.js)** — redesigned PDF builder. Branded header with logo (toner-friendly accent strip + opacity-washed Street View image when available), document reference number, hero summary card with client name + chips for each product line + bindability summary, sectioned card layout with vertical accent strips, alternating row fills in the key/value grid, page footer with Altech wordmark + page numbers + doc ref + timestamp on every page. All chrome (`setFillColor`, `rect`, `addImage`, `setGState`, `getNumberOfPages`/`setPage`, `getTextWidth`) is feature-detected so the JSDOM test mock gets a clean text fallback that still satisfies the boat HIN + RV class sentinels. Covers all four products (home/auto/boat/RV) plus operators, prior insurance, discounts, affinity, history (35 mo), follow-up appendix, and free-form agent notes.

- **feat(intakev2): genius-tier PDF snapshot — page 1 underwriting summary** (May 14, 2026):
  Layer on top of the previously-landed v2 PDF (above). The new design reorganizes the builder into compute / snapshot / detail phases. **Page 1 is now a single-page underwriting snapshot** an agent can read in under 90 seconds; pages 2+ remain the dense detail dossier.
  - **Compute phase** — pure functions `buildSnapshotModel`, `computeRiskMarkers`, `computeActionItems`, `htmlToPlain` derive everything page 1 needs. Risk markers are weighted high / med / low / pos with explicit rules (SR-22, suspended license, vacant property, lapsed coverage, roof >20y, pool, trampoline, full-timer RV, young driver, etc.). Action items merge `data.deferred` + risk-derived prompts + talk-track rules with dedupe and priority ordering.
  - **Snapshot page (page 1)** uses 7 new greyscale visual primitives, each height-bounded so the page is guaranteed to fit on letter:
    - `keystatRow` — 4-cell KPI strip (operators, years at address, continuous coverage, lines).
    - `statusGrid` — 2×2 **carrier-fit matrix** with check / X icons (drawn as 2-line strokes — no `circle`/`ellipse` calls) and `${n} missing · ${topMiss}` for not-ok cells. OK cells get the FILL tint + accent strip.
    - `severityBox` — **risk-flag callout** where severity is conveyed by greyscale density: high = 4 dark cells, med = 3 mid cells, low = 2 lite cells, positive = outline + check mark. Reads in B&W without color cues.
    - `ratioBar` — **coverage-at-a-glance** stacked bar: Cov A = 100% baseline, B/C/D rendered as proportional segments in three named greys (`GREY_DARK`/`MID`/`LITE`). Inline % labels when segment width ≥ 40pt, hanging $ amounts below. Cov E/F/deductible in a compact text block to the right.
    - `checklist` — **action-items** checkbox list, max 5 on page 1 with `+N more` overflow note, source tag (DEFER / RULE) right-aligned.
    - `pillChip` + `pillRow` — pill badges for endorsements / hazards in the home detail cards (no `roundedRect` — plain `rect` for portability).
    - `timelineStrip` — **chronological 35-month axis** with markers + tick years + numbered legend. Replaces the flat loss/violation list on the detail page.
    - `operatorAssignmentGrid` — a new section between Operators and Homes showing a **rows = operators × columns = assets** matrix with `P` (primary, filled black) or `A` (additional, outlined) cell markers. Asset columns are 5-char truncated year+make.
  - **Single-page guarantee** — after the action-items checklist the builder calls `doc.addPage()` unconditionally so the snapshot is always exactly page 1, even if future changes add rows.
  - **Footer** distinguishes page 1 (`Snapshot · Page 1 of N`) from detail pages (`Page i of N`) so agents flipping through a printed packet can find the snapshot at a glance.
  - **Appendix** — the existing "Follow-up (N)" section is renamed **"Producer Action Items — Full List"** and surfaces the complete merged action-items output (snapshot caps at 5; appendix has them all).
  - **Strict greyscale** per design choice — every status indicator readable from density alone, survives a B&W laser printer. Four new palette entries (`GREY_DARK`/`MID`/`LITE`/`POSITIVE`) — no color anywhere.
  - **Test compatibility** — every new primitive starts with `has(doc, 'rect')` guard and falls back to text-only output. The JSDOM mock at `tests/intake-v2.test.js:413-427` still gets the boat HIN + RV class sentinels because the detail phase runs unchanged after the snapshot's forced `addPage()`. **Tests: 26/26 intake-v2 + 49/49 v1 export contracts — no regressions.**
  - **[js/intake-v2-export.js](js/intake-v2-export.js)** — slimmed to an orchestrator. PDF / CMSMTF / EZLynx XML each delegate to the named builder module. Boats/RVs continue to be PDF-only (CMSMTF + V200 have no native boat/RV schemas — the orchestrator gates with a `home/auto only` toast). The legacy `intake-v2-export-map.js` is retained for `fromLegacyShape` (importing a v1 quote into v2) but no longer used on the export side.
  - **Backward compatibility**: v1 (`quoting`) tool is unchanged — every existing v1 export test still passes (49/49 across `exporter-contract`, `ezlynx-export-acord-xml`, `export-picker`). The v2 test suite still passes (26/26) — wrapper tests confirm CMSMTF + EZLynx XML files still contain the applicant + address sentinels, PDF still contains boat HIN + RV class sentinels.

- **feat(intakev2): full Smart Scan parity with v1 — 4 follow-up phases** (May 13, 2026):
  Closes the remaining gap between v1's `App.smartAutoFill()` experience and v2's Smart Scan. All four features land with deliberate improvements over their v1 counterparts (caught during exploration).
  1. **Google Places autocomplete** on every home address ([js/intake-v2-places.js](js/intake-v2-places.js) + [js/intake-v2-maps-key.js](js/intake-v2-maps-key.js)). v2 binds via `focusin` event delegation + a WeakSet guard — re-rendered cards pick up the listener without per-card re-binding (v1 leaked Autocomplete instances). v2 also collapses v1's three duplicate `/api/config?type=keys` fetches into a single shared `IntakeV2MapsKey.get()` promise-cached loader; first call from any consumer hydrates the cache, every other call awaits the same in-flight fetch.
  2. **Satellite + Street View thumbnails** embedded above the home field grid ([js/intake-v2-property-maps.js](js/intake-v2-property-maps.js)). Plain `<img src=...>` instead of v1's base64 data-URLs (~700 KB savings per home), smaller default 400×240 + 2× retina `srcset`, and a Street-View coverage probe via the free `/streetview/metadata` endpoint so rural addresses get a "No Street View coverage" placeholder instead of v1's silent 404'd broken image.
  3. **Rentcast budget pill + smarter overage modal** ([js/intake-v2-rentcast.js](js/intake-v2-rentcast.js)). Three v1 pain points fixed: pricing is read live from `/api/config?type=keys`'s new `rentcastPricing` block (env-var driven — no client redeploy when prices change), the counter increments AFTER a successful Rentcast-sourced response (v1 burned credits on 500s), and remaining budget shows as an always-visible chip under every Smart Scan button (`Rentcast: 42 of 50 left · resets Jun 1`) instead of being hidden in a settings panel. Tone flips ok→low→over as the count climbs. Audit log captures opt-in approvals immediately so consent is proven even if the API call later fails.
  4. **Vision hazard confirmation prompts** in the review modal. `extractVisionHazards()` pulls `has_pool` / `has_trampoline` / `tree_overhang_roof` / `brush_clearance_adequate` out of the vision payload and surfaces them as **unchecked** "🛰️ Vision findings" checkboxes the agent confirms one-by-one — v1 auto-applied vision booleans, building bad data when the AI guessed wrong. Per-hazard `applyTo` callbacks in `VISION_HAZARD_HANDLERS` route pool/trampoline to dedicated paths and tree/brush to the home's notes field. Confidence gate tightened from `conf !== 'low'` to require an explicit `high`/`medium` string (missing confidence is now treated as low).

  Plumbing additions: `/api/config?type=keys` returns the `rentcastPricing` block (env vars `RENTCAST_PER_CALL_USD` + `RENTCAST_FREE_LIMIT`, defaults `$0.50` / 50).

  Tests: 21 cases in `tests/intake-v2-smart-fill.test.js` (was 13) covering vision confidence tightening + `extractVisionHazards`. Full suite: intake-v2 26/26 + smart-fill 21/21 = **47/47 pass**.

- **feat(intakev2): port v1 home Smart Scan — multi-source + review modal** (May 13, 2026):
  Brings v1's `App.smartAutoFill()` orchestration into v2 home cards. Previously the "Autofill from address" button only hit one mode (zillow/Rentcast) and silently gap-filled six fields. The new Smart Scan fires four modes in parallel and presents a review modal so the agent can uncheck any value before applying. New module: `js/intake-v2-smart-fill.js`. Tests: 13 cases on `parseAddress` + `mergeResults` priority (ArcGIS-wins / Zillow gap-fill / vision confidence / boolean truth-wins / fire direct-merge / flood meta / empty sources).

- **feat(intakev2): Personal Intake v2 polish — 10-phase pass** (May 13, 2026):
  Worked through the desk-lead audit of the intake v2 workspace. Every phase committed separately so the diff stays reviewable; all tests pass after each phase.
  1. **State dropdowns** — `usStateOptions()` returns `[value, label]` tuples; renderers in intake-v2-layout/-operators/-autos/-coverage stamp `<option value="AL">Alabama (AL)</option>` so the visible label is the full state name while the stored value (and every export) stays the 2-letter USPS code.
  2. **Phonetic speller expansion** — `PhoneticSpeller.open({ seed, mode, hint, onCommit })` extended with five modes (`general`, `vin`, `dl`, `plate`, `email`). VIN mode strips spaces, upper-cases, and warns inline on I/O/Q. Email mode preserves case and reads `@`/`.`/`-`/`_` as named tokens. Apply button writes the cleaned value back when the caller passes an `onCommit` callback. New CSS `.iv2-input-wrap` + `.iv2-speller-btn` for the inset button pattern — sits inside the input's right edge, never overlaps neighbors. Wired on applicant + co-applicant names/email, address (street/city/county/prev), referral source, operator names + DL number, auto VIN + plate + lien holder, home mortgage name/loan#/address, and the home property address input. New collection fields: `auto.licensePlate` (mode='plate') and `auto.plateState` — HawkSoft expects both. Click delegation + Alt+P shortcut wired once at boot via `wireSpellerDelegation()` in intake-v2-layout.js.
  3. **Unified checkbox/switch markup** — three hand-rolled inline-styled checkbox renderers replaced with `.iv2-checkbox-row` (square) and `.iv2-switch-row` (toggle pill). Both wrap a real `<input type="checkbox">` so core's save/load handlers stay unchanged. `kind: 'switch'` on a field flips to the toggle variant. `iv2-coPresent` migrated to a switch labeled "Add Co-Applicant".
  4. **12-column grid + section card polish** — section card padding bumped 16px → 24px. New `.iv2-field-grid-12` with `data-span="N"` attributes maps to `grid-column: span N`; responsive at 720px/1024px breakpoints. Quick Start applicant row renders with the audit's Prefix(1) / First(3) / Middle(2) / Last(3) / Suffix(1) / DOB(2) spans; address row Street(6) / City(3) / State(2) / ZIP(1). New `applicant.employerName` + `applicant.yearsEmployed` fields fill the row that used to leave 4 empty cols next to Industry/Education.
  5. **3-state bindability pill** — per-carrier readiness indicator gains an "amber" state for when the agent is 1–2 fields away from a bind. `.is-ok` (green), `.is-near` (amber), `.is-miss` (red). Subtle background tint on all three so the pill reads as a real status indicator. Threshold lives in `BINDABILITY_NEAR_THRESHOLD` (=2) in intake-v2-layout.js.
  6. **Micro-interactions** — required-to-bind amber underline fires on blur when a ✦ field is empty (cleared the moment the agent types). ✓ Saved flash shows briefly after each save then auto-repaints to a muted "All changes saved" idle state, driven by `_lastSaveAt` + `SAVED_FLASH_MS=1400`. Tooltips on ✦ + 🔒 marks: ✦ → "Required to bind — at least one carrier needs this field"; 🔒 → "Synced from Quick Start — edit the primary applicant's value there".
  7. **EZLynx + HawkSoft schema audit** — 90-line comment block at the top of intake-v2-fields.js with ✅/⚠️/❌ markers across Applicant, Co-Applicant, Address, Drivers, Vehicles, Home/Dwelling, Prior Insurance, and Coverage for both targets. Boats + RVs confirmed PDF-only by design (the exporter-contract test pins this).
  8. **Driver + vehicle field gaps** — new operator fields `defensiveDriving` + `defensiveDrivingAt` (date), `matureDriver`, `mvrStatus` (select). New auto fields `purchaseDate`, `originalOwner`, `existingDamage`. New `VinDecoder.decodeForIntake(vin)` public API returns the merged local + NHTSA decode as a plain object; `decode: 'vin'` field flag stamps a "Decode VIN" pill on the auto card that auto-fills blank year/make/model/trim (never clobbers a value the agent typed).
  9. **Per-section completion %** — left rail jump buttons gain a small chip showing what fraction of each section's bindable fields the agent has captured. Colors: amber (early) → blue (≥60%) → green ✓ (100%). Implemented via `renderJumpProgress()` reading the existing bindability `missing` arrays.
  10. **A11y pass** — new `computed: true` field flag stamps `readonly tabindex="-1"` so derived fields like "Age Licensed (auto)" skip the tab order. ✦ + 🔒 marks moved from `tabindex="0"` (which added tab stops on every bindable field) to `role="img" aria-label="..."` — screen readers still get the explanation as part of the field's label, no extra tab stops.
...
- **feat(auth): forced sign-in gate for un-signed-in users** (May 13, 2026):
  ...
  - **Now:** boot pauses behind `body.auth-resolving` (centered spinner) for up to 2 s while `Auth.whenSignedIn(2000)` confirms the session. If no user resolves, `body.auth-gated` pins the existing `#authModal` as a forced sign-in screen — close button hidden, `Auth.closeModal()` no-ops, everything outside `#authModal` + `#toast` hidden by CSS allow-list. Boot early-returns so `renderLandingTools`, `Onboarding.init`, `VaultUI.maybePromptUnlockOnLoad`, `Reminders`, `DashboardWidgets.init`, the hash router, and the safety-net widget refresh are all skipped (none have anything useful without a session).
  - **Wizard moves to post-signup:** `Onboarding.init` previously fired for any first-time visitor; now it lives behind the gate's return. Brand-new visitors hit the sign-in screen first; after a fresh signup the reload from [PR #106](https://github.com/austinkays/Altech/pull/106) re-enters boot with a session, the wizard fires once, then the dashboard.
  - **Password-reset deeplinks (`?type=recovery`) bypass the redirect path** — the gate stays up but opens at the `reset` view so the user can complete recovery in-modal. After they pick a new password, Supabase fires `SIGNED_IN` → PR #106 reload → fresh gate-less boot.
  - **Command palette opt-out:** `_onGlobalKeydown` (the Cmd+K handler in [js/command-palette.js](js/command-palette.js)) returns early when `body.auth-gated` is present — the palette wires at DOMContentLoaded independently of the window.onload boot path, so it needs its own gate check. Other hotkeys (Cmd+S, App.goHome on Esc) live inside `window.onload` and never wire because of the early return.
  - **Ambient sign-out (multi-tab, session expiry):** `_onAuthStateChanged` now adds `body.auth-gated` immediately when transitioning to a null user — the ~600 ms reload bridge from PR #106 no longer shows the dashboard for that window.
  - Tests: new [tests/auth-gate.test.js](tests/auth-gate.test.js) with 15 static source-pattern tests pinning each piece (timeout budget, body-class transitions, forced-mode signature, reset deeplink, early return, MFA-safe order, CSS state machine, palette opt-out). Updated `tests/auth-reload-on-change.test.js` to allow content between `_hadSignedInUser = false` and `_scheduleAuthReload`. **61 suites, 2349 tests pass.**
...
- **feat(intake): Personal Intake v2 — phone-first ground-up rebuild for Home + Auto + Boat + RV** (May 12, 2026):
  ...
  - **New tool** `intakev2` (🧭 violet, BETA) registered in `App.toolConfig` next to legacy `quoting`. The v1 tool stays untouched so users can fall back during rollout.
  - **Speed primitives** (the things that actually make it fast on a call):
    - **Quick / Full mode toggle** — Quick mode hides every non-bindable field so the agent Tab-Tabs through only what's needed to bind. Per-field `mode: 'quick' | 'full'` in [js/intake-v2-fields.js](js/intake-v2-fields.js).
    - **Live carrier bindability indicator** in the sticky top bar — ✓/✗ per carrier, updates every keystroke, hover shows missing fields with click-to-jump. [js/intake-v2-bindability.js](js/intake-v2-bindability.js).
    - **"Ask Later" defer** — `Alt+L` on any field marks it deferred. Stays editable, ignored by bindability, surfaces in the right-rail follow-up list AND in the PDF appendix. [js/intake-v2-defer.js](js/intake-v2-defer.js).
    - **Inline-everywhere operator picker** — chip multi-select on every auto / boat / RV card with `+ New` that pops a 6-field mini-form without leaving the section. [js/intake-v2-operators.js](js/intake-v2-operators.js).
    - **Keyboard-first navigation**: `N` add (context-aware), `J/K` next/prev section, `Alt+1..6` jump, `?` help. [js/intake-v2-keyboard.js](js/intake-v2-keyboard.js).
    - **Talk-track sidebar** — scripted (not AI) prompts that surface as the conversation progresses: TCPA consent reminder, USCG Aux discount when a boat is present, defensive-driver course for teen drivers, marine-survey warning for old wood boats, TLR offer for new RVs, etc. [js/intake-v2-talktrack.js](js/intake-v2-talktrack.js).
    - **"Clean record" one-tap** — checkbox in the History section that fills the standard 35-month answer for every operator with a single click. [js/intake-v2-history.js](js/intake-v2-history.js).
    - **Per-keystroke autosave** (400 ms debounce) — encrypted to `STORAGE_KEYS.INTAKE_V2`, cloud-synced via `Sync.schedulePush()`. Per-keystroke means a dropped call loses at most one character. [js/intake-v2-core.js](js/intake-v2-core.js).
  - **Architecture** (vanilla JS, IIFE pattern, no framework, no build step):
    - `plugins/intake-v2.html` — workspace shell (top bar, left rail with jump anchors, main scrollable column with collapsible `<details>` sections, right rail for talk track + follow-up + last-entries peek, floating quick-add FAB).
    - `css/intake-v2.css` — workspace layout. Reuses existing tokens (`--bg-card`, `--apple-blue`, `--text-secondary`, `--border`, `--bg-input`). Responsive — rails collapse under 1100 px. No new design tokens.
    - 17 new `js/intake-v2-*.js` modules, each ~150-500 lines: init / fields / bindability / talktrack / core / layout / keyboard / defer / operators / autos / property / boats / rvs / coverage / history / review / export-map / export.
  - **Export strategy** — PDF intake summary is the primary export for v2 (covers all four products including boats and RVs). HawkSoft CMSMTF and EZLynx ACORD XML keep working for Home + Auto only via a translation layer [js/intake-v2-export-map.js](js/intake-v2-export-map.js) that collapses v2 nested data to the legacy flat shape, then temporarily installs the translated arrays on `App` so `App.buildCMSMTF` / `App.buildAcordXML` work untouched. Zero edits to legacy export files.
  - **Storage** — new keys in [js/storage-keys.js](js/storage-keys.js): `INTAKE_V2`, `INTAKE_V2_QUOTES`, `INTAKE_V2_ENABLED`, `INTAKE_V2_MODE`, `INTAKE_V2_RAILS`, `AGENCY_DEFAULTS`. Encrypted via existing `CryptoHelper.encrypt/decrypt` (same Phase B AAD envelope when v2 vault is unlocked on the Supabase backend).
  - **Cloud sync** — `intakeV2Draft`, `intakeV2Quotes`, `agencyDefaults` registered in both `SYNC_DOCS` and `DOC_LOCAL_KEYS` in [js/cloud-sync.js](js/cloud-sync.js) and [js/supabase-sync.js](js/supabase-sync.js), with the existing AAD-wrapping for Supabase.
  - **Tests** — new [tests/intake-v2.test.js](tests/intake-v2.test.js): bootstrap & smoke, save/load round-trip with schema migration, DOM <-> data sync, bindability for all 4 carriers (including deferred-fields-ignored), operator pool sync, defer toggle, export mapping (forward + inverse), CMSMTF & EZLynx XML wrappers (sentinel-based), boat HIN validator, PDF builder smoke. Legacy `tests/app.test.js` and `tests/exporter-contract.test.js` confirmed to still pass — zero regression surface on v1.
  - **Plan file**: `/root/.claude/plans/the-current-personal-intake-peppy-rossum.md` — the user's approved "phone-first" rebuild plan.

- **feat(sync): E2E encryption for the Firebase backend — Phase B AAD envelopes on every push** (May 11, 2026, late):
  - **Closes the remaining plaintext-on-wire exposure**. `js/cloud-sync.js` used to decrypt local ciphertext to plaintext before pushing to Firestore, relying on Firebase Auth + Firestore owner-only rules + Google's at-rest encryption as the boundary. With this change, when the v2 vault is unlocked, every Firestore push (sync docs + quotes) is wrapped in a v=2 AAD envelope bound to `(table, rowId, userId)` — mirroring the Supabase `pushBlob` / `pushQuote` path. Firestore now sees ciphertext for the docs that used to be plaintext (`cglState`, `reminders`, `clientHistory`, `quickRef*`, `glossary`, `carrierOverrides`, `settings`, `commercialDraft`/`commercialQuotes`, `currentForm`, `vaultMeta`).
  - **New helpers** at the top of `cloud-sync.js`: `_wrapForFirestore(payload, table, rowId)` / `_unwrapFromFirestore(value, table, rowId)`. The wrap throws `CRYPTO_LOCKED` if v2 is enrolled but locked — the sync is refused rather than silently downgraded to plaintext. When v2 isn't enrolled at all, the legacy plaintext path is preserved so backward compat for non-migrated users is intact.
  - **Quote shape change**: when v2 wraps, each quote doc becomes `{ payload: <envelope>, updatedAt, deviceId }` instead of `{ ...quote, updatedAt, deviceId }`. `_pullQuotes` detects either shape and unwraps as appropriate, so the lazy-upgrade window works in both directions.
  - **Conflict detection**: `_pushDoc` decrypts the remote `data` via `_unwrapFromFirestore` before comparing to local — otherwise every fresh-IV push would look like a conflict on identical data.
  - **`window.CloudSync` is now also assigned explicitly** at the bottom of `cloud-sync.js` (matching `SupabaseSync`'s pattern) so module-style tests can `require` it the same way.
  - **Tests**: new [tests/cloud-sync-aad.test.js](tests/cloud-sync-aad.test.js) — 10 tests covering: envelope wrap on push, AAD binding (refuses cross-row/cross-user reads), encrypted-localStorage round-trip, refusal-on-locked-vault, legacy plaintext pass-through, pull-unwrap, locked-vault drop, quote shape, quotes round-trip. Updated 4 assertions in `tests/sync-conflict-ui.test.js` + `tests/hotfix-429-and-scrub.test.js` (the push body now reads `data: wrapped` and the conflict descriptor's `remoteData` is the unwrapped plaintext). **2314/2314 tests pass** across 59 suites.

### Fixed
- **fix(auth): the bridge guard was a one-character typo + `_authReady` poisoning** (May 12, 2026, follow-up to the earlier fix in the same entry):
  - **Symptom**: PR #98 only fixed half the bug. After deploying it, F5 from `#quoting` still landed on the Welcome Back modal even though the Supabase session was valid in localStorage and `SupabaseAuth.user` was populated. Chrome-Claude reproduced it and traced the residual race.
  - **Root cause**: The bridge guard in `Auth.init` read `if (event === 'INITIAL' && !sbUser) return;`. The Supabase JS v2 SDK fires `'INITIAL_SESSION'` (note: `_SESSION` suffix), NOT `'INITIAL'` — only `SupabaseAuth.addAuthListener`'s own synthetic registration event uses `'INITIAL'`. The guard caught the synthetic but missed the SDK's null `INITIAL_SESSION` that fires when the listener attaches before hydration finishes. That null leaked through, called `_onAuthStateChanged(null)`, and (one-shot) resolved `Auth._authReady` with null. The auth gate in `app-navigation` awaited `Auth.ready()`, got null, and opened the Welcome Back modal even though a real user event arrived a few hundred ms later. PR #98's `SupabaseAuth.ready()` mirror eventually populated `_user`, but `_authReady` was already poisoned and `Auth.showModal()` had already fired.
  - **Secondary race**: Even with the bridge guard fixed, Firebase's `onAuthStateChanged` fires `null` for any Supabase-only account (no Firebase identity exists) and that null ALSO leaked into `_authReady` via `_onAuthStateChanged`. PR #98's bridge-only fix wouldn't have caught it.
  - **Fix in [js/auth.js](js/auth.js)** — two changes:
    - **Conservative bridge guard**: `if (!sbUser && event !== 'SIGNED_OUT') return;` — any null event from the Supabase bridge is now ignored unless it explicitly accompanies a sign-out. Forward-compatible against future SDK event names.
    - **`_authReady` only resolves on a positive signal**: `if (_authReadyResolve && user) { _authReadyResolve(user); _authReadyResolve = null; }`. The pre-fix unconditional resolve poisoned `_authReady` on the first null from either backend. The 15s safety timeout still catches genuinely-unauthenticated cold loads, so callers don't hang forever — worst case is a 15s delay before the sign-in modal appears for an actually-signed-out user on first tool-tile click, in exchange for authenticated users no longer getting falsely logged out on F5.
  - **Regression coverage**: new [tests/auth-bridge-race.test.js](tests/auth-bridge-race.test.js) — 4 static source-pattern assertions that pin the conservative guard, the only-resolve-on-user resolve, the 15s timeout, and the belt-and-suspenders mirror against `SupabaseAuth.user`. Behavioral tests would need a heavier SDK mock; the static guards keep the one-character typo from coming back.
  - **All 66 suites / 2405 tests pass**. AAD lint clean.

- **fix(auth): close the cold-load bridge race that kept `Auth.user` null for Supabase users** (May 12, 2026):
  - **Symptom**: After signing in on the Supabase backend, the session would not survive an F5 — Welcome Back reappeared, the dashboard greeting fell through to "Good afternoon" with no name, and clicking tool tiles re-opened the sign-in modal. The toast "Signed in as …" fired correctly, so login itself was working; the Supabase persisted session WAS restored on cold load; only `Auth.user` (the backend-agnostic facade everything else reads) stayed null.
  - **Root cause** in [js/auth.js](js/auth.js): `app-boot.js` calls `Auth.init()` BEFORE `SupabaseAuth.init()`. The bridge listener registered inside `Auth.init` fires synchronously with `(null, 'INITIAL')` because `SupabaseAuth._user` is not yet hydrated — the early-return guard from PR #90 correctly swallows it. Later, `SupabaseAuth.init()` awaits `getSession()`, populates `_user`, then registers `client.auth.onAuthStateChange`. The Supabase JS v2 SDK is *supposed* to fire `INITIAL_SESSION` for the restored session at that point, but the synthetic restore event sometimes never reaches `_listeners.forEach` (timing of `_refreshFactors` vs. the SDK queue). Once the bridge missed that one event, `Auth._user` stayed null for the rest of the session.
  - **Fix**: Belt-and-suspenders mirror after `addAuthListener` — if `SupabaseAuth.user` is already populated, mirror immediately; otherwise wait for `SupabaseAuth.ready()` (the existing 5s-safety-timeout promise that resolves when `getSession` settles) and mirror once. A `!_user` guard inside the deferred branch avoids clobbering a real event that arrived in between. The `is_blocked` defensive admin path on line ~109 of [js/supabase-auth.js](js/supabase-auth.js) is unchanged — it was never the cause (the check is `=== true`, so an unset metadata field is safely a no-op).
  - **Secondary fix** in [js/dashboard-widgets.js](js/dashboard-widgets.js): `renderGreeting()` (the function that actually paints the visible `#dashboardGreeting` header) still read `firebase.auth().currentUser` directly, so even after the bridge race is closed the greeting would still show no name for Supabase-only accounts. PR #96 fixed the parallel `App.updateLandingGreeting` in app-navigation.js — but that function targets `#landingGreeting` which doesn't exist on the current dashboard, so it's a dead path. Mirrored the same `Auth.user`-first lookup into `renderGreeting`.
  - **All 65 suites / 2401 tests pass**. AAD lint clean. No new tests required — the existing `auth-cloudsync.test.js` suite mocks `firebase` but not `SupabaseAuth`, so the bridge code path is conditionally skipped in the test environment and the new mirror block lives entirely inside that conditional.
...
- **feat(reminders): time-of-day, every-N-days frequency, undo-complete toast** (May 11, 2026):
  ...
  - **Undo-complete toast** ([js/reminders.js](js/reminders.js), [css/reminders.css](css/reminders.css)). Replaces the silent celebrate-and-advance flow. After completing a task, a 5-second toast shows " ✓ Done! [Undo]". Clicking Undo pops the completion entry AND restores the pre-completion `dueDate` (so a recurring task's cycle isn't accidentally advanced). Uses the existing `App.toast` queue (with `useHtml: true`) so it sits alongside other toasts cleanly. Titles are escaped via `Utils.escapeHTML`.
  - **Tests**: new [tests/reminders-enhancements.test.js](tests/reminders-enhancements.test.js) — 19 tests covering time normalization (24h + 12h + garbage), `_format12hour` rollovers, frequency wiring at `addTask`/`updateTask`/`_autoAdvanceRecurring`/`_getNextDueDate`/`saveEdit`, undo expiry, pre-completion dueDate restore, and modal HTML wiring. **2251/2251 tests pass** across 54 suites (was 2232/53).

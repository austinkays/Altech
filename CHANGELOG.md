...
### Added
- **ci: GitHub Actions test gate — the suite now actually blocks regressions** (May 19, 2026):
  Until now the *only* PR/deploy check was the Vercel preview build — the ~71-suite Jest run never executed in CI, so a regression (or the long-ignored failing test) could ship to `main` unnoticed. New [.github/workflows/test.yml](.github/workflows/test.yml) runs on every PR + on pushes to non-`main` branches (main skipped — post-merge, Vercel handles it): `npm ci` → `npm run audit-docs` (doc/size budget — note `pretest` only runs `lint-aad`, audit-docs is separate so CI runs it explicitly) → `npm test` (`pretest` lint-aad + jest), Node 22, 25-min timeout, `concurrency` cancels superseded runs. Paired with the **intake-v2 bindability test fix below** so the new gate is green from day one rather than red-flagging every PR. Status check only — does not auto-merge or touch the Vercel flow.

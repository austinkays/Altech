...
// State centroids for location-biased search (lat, lng, radius in meters)
...
function _extractStateFromAddress(address) {
  if (!address) return '';
  // Match ", ST ZIPCODE" pattern (US addresses)
  const match = address.match(/,\s*([A-Z]{2})\s+\d{5}/);
  if (match) return match[1];
  // Fallback: split by comma, look for state-like segments
...
async function handlePlacesLookup(query) {
  ...
  try {
    // Build search query with state name for better targeting
    ...
    // Add location bias for the selected state
...
function _extractCityFromAddress(address) {
  if (!address) return '';
  const parts = address.split(',').map(s => s.trim());
  // City is usually the second-to-last part before "STATE ZIP" and "COUNTRY"
...
/**
 * Get full Place Details for a known placeId.
 * Used in both standard lookup and Phase 2 direct lookup.
 */

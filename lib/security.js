...
/**
 * Extract and verify the bearer token from the Authorization header against
 * Supabase first (the primary backend as of Phase D, May 2026), falling back
 * to the legacy Firebase verification only when no Supabase config is
 * available OR Supabase rejects the token.
 *
 * Returns a normalized `{ uid, localId, email }` object on success (the
 * `localId` alias preserves the previous Firebase return shape so existing
 * callers like `req.uid = user.localId` keep working unchanged) — or null.
 */
export async function verifyAuthToken(req) {
  ...
  // Try Supabase first. Uses the Supabase REST API's getUser endpoint via
  // the user's own access token — no service role key needed, no JWT secret
  // needed. The Supabase URL is required.
    ...
    try {
      ...
      if (resp.ok) {
        ...
        if (data && data.id) {
          return {
            uid: data.id,
            localId: data.id, // alias for legacy Firebase callers
            email: data.email || '',
            isAdmin: !!(data.app_metadata && data.app_metadata.is_admin === true),
            backend: 'supabase',
          };
        }
      }
    } catch {
      // Network or parse error — fall through to Firebase as last resort.
    }
  }

  // Legacy: Firebase token. Kept as a fallback for the one-release Phase D
  // rollout window so any user who explicitly opts back to Firebase via
  // localStorage[altech_sync_backend]='firebase' can still hit the API.
  return verifyFirebaseToken(req);
}

/**
 * Legacy Firebase-only verifier. Kept exported because some endpoints (notably
 * config.js's bug-report handler) still import the original name. New code
 * should use `verifyAuthToken` which tries Supabase first.
 *
 * Uses the Firebase Auth REST API (accounts:lookup) — no firebase-admin needed.
 * Requires FIREBASE_API_KEY env var when the user is on the Firebase backend.
 */
export async function verifyFirebaseToken(req) {
  ...
  try {
    ...
    if (!user) return null;
    return { ...user, uid: user.localId, backend: 'firebase' };
  } catch {
    return null;
  }
}
...
/**
 * Wraps a handler to require a valid auth token (Supabase OR Firebase).

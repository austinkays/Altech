 ...
 * Supabase. Returns a normalized `{ uid, localId, email, isAdmin }` object on
 * success (the `localId` alias preserves the shape older callers used when
 * Firebase was the backend, so `req.uid = user.localId` still works) — or null.
 */
export async function verifyAuthToken(req) {
  ...
  try {
    ...
    return {
      uid: data.id,
      localId: data.id, // alias preserved for legacy callers
      email: data.email || '',
...
/**
 * Wraps a handler to require a valid auth token. Returns 401 if missing or
 * invalid. Injects the verified user as req.user / req.uid / req.userEmail.
 */

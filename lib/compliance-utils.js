...
export function getCommercialPolicyType(policy) {
  ...
  if (/workers|\bwc\b|work comp/i.test(allText)) return 'wc';
  ...
  if (/inland(?:\s+marine)?|\bim\b/i.test(allText)) return 'im';
  ...
  if (/prpty|commercial property|comm prop|\bcp\b|bldg|building/i.test(allText)) return 'property';
...
export function getPersonalPolicyType(policy) {
  ...
  if (/homeowner|ho\d/i.test(allText) || pn.startsWith('HO') || pn.startsWith('OP')) return 'homeowner';

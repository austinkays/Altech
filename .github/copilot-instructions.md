...
**Stack:** Vanilla JS SPA (`index.html` ~742 lines), 34 CSS files in `css/` (~21,082 lines), 47 JS modules in `js/` (~40,530 lines), 22 plugin HTML files in `plugins/` (~6,808 lines), 12 serverless APIs in `api/` (~6,307 lines). Firebase Auth + Firestore for cloud sync. Deployed to Vercel.

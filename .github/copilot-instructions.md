...
**Stack:** Vanilla JS SPA (`index.html` ~742 lines), 34 CSS files in `css/` (~22,223 lines), 46 JS modules in `js/` (~42,657 lines), 21 plugin HTML files in `plugins/` (~6,712 lines), 12 serverless APIs in `api/` (~6,307 lines). Firebase Auth + Firestore for cloud sync. Deployed to Vercel.

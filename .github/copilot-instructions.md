...
**Stack:** Vanilla JS SPA (`index.html` ~742 lines), 34 CSS files in `css/` (~22,946 lines), 46 JS modules in `js/` (~43,510 lines), 21 plugin HTML files in `plugins/` (~6,712 lines), 12 serverless APIs in `api/` (~6,307 lines). Firebase Auth + Firestore for cloud sync. Deployed to Vercel.

> **Full documentation:** See [AGENTS.md](../AGENTS.md) and [QUICKREF.md](../QUICKREF.md) for complete architecture reference.

```bash
npm run dev          # Local server
npm test             # 67 test suites, ~2,530 tests (Jest + JSDOM)
...
```bash
npm test                    # All 67 suites, ~2,530 tests
npx jest --no-coverage      # Faster (skip coverage)
